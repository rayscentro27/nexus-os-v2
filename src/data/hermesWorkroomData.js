...
function respondCasualPersonal() {
  const options = [
...
function respondNexusTopic(topicKey) {
...
function respondQuestion(text) {
  const lower = text.toLowerCase();

  // Try to find a matching Nexus topic
  for (const [pattern, topic] of Object.entries(nexusTopics)) {
    if (lower.includes(pattern)) {
      return respondNexusTopic(pattern);
    }
  }
  ...
  return respondConversation(text);
}

function respondConversation(text) {
  const options = [
...
export function buildHermesResponse(message, specialist = 'Hermes CEO Advisor') {
  ...
  switch (intent.type) {
    case 'greeting':
      response = respondGreeting();
      break;
    case 'casual':
      response = respondCasual();
      break;
    case 'casual_personal':
      response = respondCasualPersonal();
      break;
    case 'emotional':
      response = respondEmotional();
      break;
    case 'partner_mode':
      response = respondPartnerMode();
      break;
    case 'nexus_topic':
      response = respondNexusTopic(intent.topic);
      break;
    case 'opinion':
      response = respondOpinion();
      break;
    case 'approval':
      response = respondApproval();
      break;
    case 'money':
      response = respondMoney();
      break;
    case 'blockers':
      response = respondBlockers();
      break;
    case 'strategy':
      response = respondStrategy();
      break;
    case 'delegation':
      queued = true;
      response = respondDelegation();
      break;
    case 'summary':
      response = respondSummary();
      break;
    case 'trading':
      response = respondTrading();
      break;
    case 'question':
      response = respondQuestion(text);
      break;
    case 'conversation':
    default:
      response = respondConversation(text);
      break;
  }

  return { intent: intent.type, specialist, text: response, queued, context: hermesContext };
}
