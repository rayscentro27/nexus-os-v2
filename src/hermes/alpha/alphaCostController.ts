...
export type AlphaCostMode='cheap'|'fast'|'strategy'|'search'|'url_review'|'deep';
export type AlphaUsageRecord={date:string;localCalls:number;hostedCalls:number;searchCalls:number;urlReviewCalls:number;estimatedInputTokens:number;estimatedOutputTokens:number;estimatedSpendUsd:number|null;blockedDeepCalls:number;fallbackCount:number;cacheHits:number;providers:Partial<Record<AlphaProviderName,number>>};
const KEY='nexus-alpha-usage-v2',MODE_KEY='nexus-alpha-cost-mode-v1';
export const MAX_HOSTED_CALLS_FALLBACK=25,MAX_SEARCH_RESULTS=5,MAX_SEARCH_CALLS_PER_DAY=10,MAX_URL_REVIEW_CALLS_PER_DAY=10;
const day=()=>new Date().toISOString().slice(0,10);const fresh=():AlphaUsageRecord=>({date:day(),localCalls:0,hostedCalls:0,searchCalls:0,urlReviewCalls:0,estimatedInputTokens:0,estimatedOutputTokens:0,estimatedSpendUsd:null,blockedDeepCalls:0,fallbackCount:0,cacheHits:0,providers:{}});
...
export function recordUrlReview(){const x=loadAlphaUsage();x.urlReviewCalls++;return save(x)}
...
export function canUrlReview(){return loadAlphaUsage().urlReviewCalls<MAX_URL_REVIEW_CALLS_PER_DAY}
export function providerAllowedForMode(provider:AlphaProviderName,mode:AlphaCostMode){if(mode==='cheap')return provider==='deterministic_local'||provider==='ollama_local';if(mode==='fast')return provider==='groq'||provider==='deterministic_local'||provider==='ollama_local';if(mode==='strategy'||mode==='search'||mode==='url_review')return true;return false}
export function getAlphaUsageSummary(maxHosted=MAX_HOSTED_CALLS_FALLBACK){const x=loadAlphaUsage();return {...x,maxHosted,maxSearch:MAX_SEARCH_CALLS_PER_DAY,maxUrlReviews:MAX_URL_REVIEW_CALLS_PER_DAY,totalTokens:x.estimatedInputTokens+x.estimatedOutputTokens,estimatedSpendLabel:x.estimatedSpendUsd===null?'unknown estimate':`$${x.estimatedSpendUsd.toFixed(4)} estimated`,deepModeUnlocked:false,policy:'One message = max 1 hosted call + 1 search/url-review call; no autonomous loops.'}}
