...
import { Activity, ArrowRight, Bot, BriefcaseBusiness, CheckCircle2, ChevronRight, CircleAlert, FileText, FolderKanban, HeartPulse, Layers3, Menu, Network, Plus, Search, SearchCheck, Settings2, Sparkles, Target, TrendingUp, UsersRound, X } from 'lucide-react'
...
import AutonomyProofConsole from '../components/AutonomyProofConsole'
...
import AdminOperatingCenter from '../components/AdminOperatingCenter'
...
const nav = [
  ...
  { id: 'live-intelligence', label: 'Live Intelligence', icon: Activity },
...
function routeFromLocation() {
  ...
  if (path === '/admin/review') return { area: 'studio', subpage: 'review', agent: getStoredAgent(), conversationId: null }
  if (path === '/admin/research') return { area: 'studio', subpage: 'research', agent: getStoredAgent(), conversationId: null }
...
const secondaryNav = {
  business: [['Overview', 'business'], ['Clients', 'business-clients'], ['Credit & Funding', 'business-credit'], ['Revenue', 'business'], ['Opportunities', 'business-opportunities'], ['Growth', 'business']],
...
function NavigationContext({ area, subpage, onNavigate }) {
  ...
  const tabs = secondaryNav[area]
  return <div className="nx2-navigation-context"><div className="nx2-breadcrumb" data-testid="nx2-breadcrumb"><button type="button" onClick={() => onNavigate('command')}>NEXUS</button><span>/</span><button type="button" onClick={() => onNavigate(area)}>{areaNames[area].toUpperCase()}</button>{subpage && <><span>/</span><span aria-current="page">{subpageNames[subpage] || subpage}</span></>}</div>{subpage && <button type="button" className="nx2-back-action" data-testid="nx2-back" onClick={() => onNavigate(area)}>← Back to {areaNames[area]}</button>}{tabs && <nav className="nx2-secondary-nav" data-testid="nx2-secondary-nav" aria-label={`${areaNames[area]} navigation`}>{tabs.map(([label, target]) => <button type="button" className={target === area || target.endsWith(`-${subpage}`) || (target === `${area}-${subpage}`) ? 'active' : ''} key={`${label}-${target}`} onClick={() => onNavigate(target)}>{label}</button>)}</nav>}</div>
}
...
function StudioPage({ subpage, onNavigate, email }) { const body = subpage === 'research' ? <ResearchNotebookWorkspace email={email} onNavigate={(target) => onNavigate(target === 'command' ? 'command' : 'studio')} /> : subpage === 'review' ? <AdminOperatingCenter /> : subpage === 'campaigns' ? <MarketingDraftsPanel onAskHermes={() => onNavigate('agents')} /> : <><CreativeReviewStudio /><Section title="Studio destinations"><div className="nx2-link-grid">{['Review','Research','Creative','Campaigns','Artifacts','Reports'].map(x => <button key={x} onClick={() => onNavigate(x === 'Review' ? 'studio-review' : x === 'Research' ? 'studio-research' : x === 'Campaigns' ? 'studio-campaigns' : 'studio')}>{x}<ArrowRight size={14} /></button>)}</div></Section></>; return <div className="nx2-page"><NavigationContext area="studio" subpage={subpage} onNavigate={onNavigate} /><div className="nx2-hero"><div><div className="nx2-eyebrow">STUDIO / {subpage ? subpage.toUpperCase() : 'WORKSPACE'}</div><h2>{subpage || 'Make the work visible.'}</h2><p>Review verified Creative artifacts, compare versions, and record internal decisions.</p></div></div>{body}</div> }
...
function SystemPage({ subpage, email, onNavigate }) { const body = subpage === 'mission-control' ? <HermesMissionControlV2 email={email} onNavigate={onNavigate} /> : subpage === 'health' ? <SystemHealthPanel onNavigate={onNavigate} onAskHermes={() => onNavigate('agents')} /> : subpage === 'workers' ? <AutomationSchedulerPanel onOpenReport={() => onNavigate('studio')} onReview={() => onNavigate('work')} /> : <><AutonomyProofConsole /><div className="nx2-kpi-row"><Kpi label="Core" value="Evidence required" note="Canonical telemetry above" tone="amber" /><Kpi label="Optional capacity" value="Deferred" note="GPU worker remains deferred" tone="amber" /><Kpi label="Runtime cost" value="UNKNOWN" note="No source, no zero" tone="blue" /></div><Section title="System destinations"><div className="nx2-link-grid">{['Mission Control','Workers','Integrations','Costs','Runtime','Diagnostics'].map(x => <button key={x} onClick={() => onNavigate(x === 'Mission Control' ? 'system-mission-control' : x === 'Workers' ? 'system-workers' : 'system')}>{x}<ArrowRight size={14} /></button>)}</div></Section></>; return <div className="nx2-page"><NavigationContext area="system" subpage={subpage} onNavigate={onNavigate} /><div className="nx2-hero"><div><div className="nx2-eyebrow">SYSTEM / {subpage ? subpage.toUpperCase() : 'OVERVIEW'}</div><h2>{subpage || 'Is Nexus machinery working?'}</h2><p>Human-readable summary first. Technical evidence second.</p></div></div>{body}</div> }

export default function NexusExperienceAdmin({ email, initialPage = 'command' }) {
  const initial = useMemo(routeFromLocation, [])
  const [area, setArea] = useState(initial.area)
  const [subpage, setSubpage] = useState(initial.subpage || null)
  ...
  function onConversationChange(id, nextAgent) { if (nextAgent) { openAgent(nextAgent, id); return } setConversationId(id); window.history.replaceState({}, '', `/admin/agents/${selectedAgent}/chat/${id}`) }
  function askAgent(agent = 'hermes', initialPrompt = '') { openAgent(agent, null, initialPrompt) }
  useEffect(() => { const sync = () => { const next = routeFromLocation(); setArea(next.area); setSelectedAgent(next.agent); setConversationId(next.conversationId); setSubpage(next.subpage || null); setPendingPrompt(''); setGlobalComposerOpen(false) }; window.addEventListener('popstate', sync); window.addEventListener('hashchange', sync); return () => { window.removeEventListener('popstate', sync); window.removeEventListener('hashchange', sync) } }, [])

  function globalContext() { return `Context: ${areaNames[area] || 'Nexus'}${subpage ? ` / ${subpageNames[subpage] || subpage}` : ''}` }
  function GlobalAskNexus() { if (area === 'agents') return null; return <div className="nx2-global-ask-dock" data-testid="nx2-global-ask">{globalComposerOpen ? <div className="nx2-global-ask-panel"><div className="nx2-global-ask-head"><strong>Ask Nexus</strong><button type="button" onClick={() => setGlobalComposerOpen(false)} aria-label="Close Ask Nexus"><X size={16} /></button></div><NexusAgentConversation key={`global-${selectedAgent || 'hermes'}`} agent={selectedAgent || 'hermes'} context={globalContext()} onConversationChange={(id, next) => { if (next) { setSelectedAgent(next); setStoredAgent(next) } }} /></div> : <button type="button" className="nx2-global-ask-button" onClick={() => setGlobalComposerOpen(true)}>Ask Nexus <span>⌘K</span><Sparkles size={16} /></button>}</div> }
