...
export default function NexusExperienceAdmin({ email, initialPage = 'command' }) {
  ...
  function GlobalAskNexus() { if (area === 'agents') return null; return <div className="nx2-global-ask-dock" data-testid="nx2-global-ask">{globalComposerOpen ? <div className="nx2-global-ask-panel"><div className="nx2-global-ask-head"><strong>Ask Nexus</strong><button type="button" onClick={() => setGlobalComposerOpen(false)} aria-label="Close Ask Nexus"><X size={16} /></button></div><NexusUniversalComposer agent={selectedAgent || 'hermes'} onAgentChange={next => { setSelectedAgent(next); setStoredAgent(next) }} context={globalContext()} onSend={text => askAgent(selectedAgent || 'hermes', text)} /></div> : <button type="button" className="nx2-global-ask-button" onClick={() => setGlobalComposerOpen(true)}>Ask Nexus <span>⌘K</span><Sparkles size={16} /></button>}</div> }
