...
function StudioPage({ subpage, onNavigate }) { const body = subpage === 'research' ? <ResearchEnginePanel onAskHermes={() => onNavigate('agents')} /> : subpage === 'campaigns' ? <MarketingDraftsPanel onAskHermes={() => onNavigate('agents')} /> : <><NexusCreativeStudioWorkspace onAskHermes={() => onNavigate('agents')} onReview={() => onNavigate('ray-review')} /><Section title="Studio destinations"><div className="nx2-link-grid">{['Research','Creative','Campaigns','Artifacts','Reports'].map(x => <button key={x} onClick={() => onNavigate(x === 'Research' ? 'studio-research' : x === 'Campaigns' ? 'studio-campaigns' : 'studio')}>{x}<ArrowRight size={14} /></button>)}</div></Section></>; return <div className="nx2-page"><NavigationContext area="studio" subpage={subpage} onNavigate={onNavigate} /><div className="nx2-hero"><div><div className="nx2-eyebrow">STUDIO / {subpage ? subpage.toUpperCase() : 'WORKSPACE'}</div><h2>{subpage || 'Make the work visible.'}</h2><p>Explore, compare, critique, and iterate on outputs while canonical Creative Intelligence remains authoritative.</p></div></div>{body}</div> }
