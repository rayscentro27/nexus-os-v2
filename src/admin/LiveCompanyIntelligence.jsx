...
import GoclearBetaProjectionPanel from '../components/GoclearBetaProjectionPanel'
import MarketingCampaignCertificationPanel from '../components/MarketingCampaignCertificationPanel'
...
function DetailRow({ icon, title, detail, meta, tone = 'cyan', onClick }) {
  return <div className="live-command-detail-row" role={onClick ? 'button' : undefined} tabIndex={onClick ? 0 : undefined} onClick={onClick} onKeyDown={event => onClick && event.key === 'Enter' && onClick()}><span className={`live-command-row-icon tone-${tone}`}><Icon name={icon} size={14} /></span><div><strong>{title}</strong><small>{detail}</small></div>{meta && <em>{meta}</em>}</div>
}
...
async function adminMutationFetch(input, init = {}) {
  if (!supabase) throw new Error('admin_session_required')
  const { data, error } = await supabase.auth.getSession()
  const token = data.session?.access_token
  if (error || !token) throw new Error('admin_session_required')
  return fetch(input, { ...init, headers: { ...(init.headers || {}), Authorization: `Bearer ${token}` } })
}

function ProjectSurface({ state }) {
  const projects = state.campaign_list || []
  const [tab, setTab] = useState('All'); const [query, setQuery] = useState(''); const [selected, setSelected] = useState(null)
  ...
  return <div className="live-surface-page"><SurfaceHeader eyebrow="WORK / PROJECT PORTFOLIO" title="Projects" description="Turn ideas into real progress." quote="Progress you can see." /><div className="live-surface-toolbar"><div className="live-surface-tabs">{['All', 'Active', 'Planning', 'Completed'].map(item => <button type="button" className={tab === item ? 'active' : ''} onClick={() => setTab(item)} key={item}>{item}</button>)}</div><div className="live-surface-toolbar-actions"><label><Search size={14} /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search projects" aria-label="Search projects" /></label><button type="button" className="live-surface-primary" disabled title="NOT_YET_AVAILABLE: no governed project-creation write path"><FolderPlus size={14} /> New Project</button></div></div><div className="live-project-grid">{visibleProjects.map((project, index) => <button type="button" className={`live-project-card accent-${['green', 'blue', 'purple', 'amber'][index % 4]}`} key={project.name} onClick={() => setSelected(project)}><div className="live-project-card-top"><span>{project.status || 'STATUS UNAVAILABLE'}</span><Target size={16} /></div><h3>{project.name}</h3><p>{project.next || 'Project detail is not currently available.'}</p><footer><span>Team · {project.department || 'Unavailable'}</span><span>Evidence · click to inspect</span></footer></button>)}</div>{selected && <CommandPanel title={`Project detail · ${selected.name}`} eyebrow="SELECTED OBJECT"><p className="live-surface-copy">Status: {selected.status || 'Unavailable'} · Owner: {selected.department || 'Unavailable'} · Next action: {selected.next || 'Unavailable'}</p><button type="button" onClick={() => setSelected(null)}>Close detail</button></CommandPanel>}{!visibleProjects.length && <div className="live-honest-empty"><FolderKanban size={20} /><strong>No matching project records currently available</strong><span>Live project data will appear here when the governed source returns it.</span></div>}<div className="live-project-overview"><CommandPanel title="Project Health Overview" eyebrow="PORTFOLIO HEALTH"><div className="live-project-health"><div className="live-health-donut"><strong>{projects.length}</strong><small>Projects</small></div><div className="live-health-kpis"><SurfaceStat label="Active Projects" value={state.campaigns?.active} tone="teal" /><SurfaceStat label="Completed" value={state.campaigns?.completed} tone="purple" /></div></div></CommandPanel></div></div>
}
...
function DecisionsSurface({ state }) {
  const [items, setItems] = React.useState(state.ray_decisions?.items || []); const [selected, setSelected] = React.useState(null); const [feedback, setFeedback] = React.useState(''); const [message, setMessage] = React.useState(''); const [busy, setBusy] = React.useState(false)
  React.useEffect(() => { setItems(state.ray_decisions?.items || []) }, [state.ray_decisions?.items])
  async function decide(item, decision) { setBusy(true); setMessage('Saving governed decision…'); try { const response = await adminMutationFetch('/.netlify/functions/company-cycle-decision', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ approval_id: item.approval_id || item.review_item_id, decision, feedback }) }); const result = await response.json(); if (!response.ok) throw new Error(result.error || 'decision_failed'); setItems(current => current.map(row => (row.approval_id || row.review_item_id) === (item.approval_id || item.review_item_id) ? { ...row, status: decision === 'approve' ? 'APPROVED' : decision === 'reject' ? 'REJECTED' : 'REQUEST_CHANGES' } : row)); setMessage(`Decision recorded: ${result.receipt?.receipt_id || decision}.`); setSelected(null) } catch (error) { setMessage(`Decision not recorded: ${error.message}`) } finally { setBusy(false) } }
  return <div className="live-surface-page"><SurfaceHeader eyebrow="GOVERNANCE / HUMAN GATE" title="Ray Decision Queue" description="Review and control governed actions before Nexus continues." quote="Clarity at the right moment." /><div className="live-decision-continuation"><span><i /> Global machine work remains active</span><small>Human review is item-level; approval advances the existing company cycle only.</small></div><div className="live-review-source">{state.review_data_source === 'GOVERNED_LIVE_READ_MODEL' ? 'Authenticated governed read model' : 'Canonical local approval projection'}</div><div className="live-decision-grid">{items.map((item, i) => <CommandPanel key={item.review_item_id || item.id || i} title={item.title || 'Decision item'} eyebrow={item.type || 'REVIEW'}><button type="button" aria-label={item.title || 'Decision item'} className="live-decision-card-button" onClick={() => setSelected(selected?.review_item_id === item.review_item_id ? null : item)}><div className="live-decision-meta"><SurfaceStat label="Approval" value={item.approval_id || item.review_item_id || '—'} /><SurfaceStat label="Cycle" value={item.company_cycle_id || '—'} tone="purple" /><SurfaceStat label="Status" value={item.status || 'PENDING'} tone="amber" /></div><p className="live-surface-copy">{item.why_human_review_required || item.reason || 'Ray review is required for this item.'}</p></button>{selected?.review_item_id === item.review_item_id && <div className="live-decision-expanded"><div className="live-decision-detail"><strong>Ray is deciding</strong><span>{item.what_ray_is_deciding || 'Review the bounded action.'}</span></div><p>Risk: {item.risk || 'HIGH'} · Scope: {JSON.stringify(item.scope || {})}</p><p>Evidence: {(item.artifact_refs || []).join(', ') || 'Package references unavailable'}</p><p>{item.external_action_if_approved || 'External execution remains receipt-gated.'}</p><textarea value={feedback} onChange={event => setFeedback(event.target.value)} placeholder="Optional feedback or requested changes" aria-label="Decision feedback" /><div className="live-decision-actions"><button type="button" disabled={busy} onClick={() => decide(item, 'approve')}>APPROVE</button><button type="button" disabled={busy} onClick={() => decide(item, 'reject')}>REJECT</button><button type="button" disabled={busy} onClick={() => decide(item, 'request_changes')}>REQUEST CHANGES</button><button type="button" onClick={() => { window.location.hash = '/ai-command'; window.dispatchEvent(new CustomEvent('nova-decision-context', { detail: item })) }}>ASK NOVA</button></div></div>}</CommandPanel>)}{!items.length && <CommandPanel title="No decisions waiting" eyebrow="CURRENT QUEUE"><div className="live-honest-empty"><CheckSquare size={20} /><strong>No decisions waiting</strong><span>The governed approval projection returned no actionable items.</span></div></CommandPanel>}</div>{message && <div className="live-decision-message" role="status">{message}</div>}</div>
}

function AiCommandSurface({ state }) {
  return <div className="live-surface-page live-ai-page"><SurfaceHeader eyebrow="COMMAND / NOVA" title="AI Command" description="A dependable conversation space for questions, decisions, and next actions." quote="Turn your questions into progress." /><GoclearBetaProjectionPanel surface="nova" /><MarketingCampaignCertificationPanel surface="nova" /><NexusAgentConversation agent="nova" /></div>
}

const existingCampaignApproval = { review_item_id: 'appr_f1c900e26b494b71b576877b9019b891', approval_id: 'appr_f1c900e26b494b71b576877b9019b891', company_cycle_id: 'company_cycle_ddc1b360d7a64a3085b819bec48554f1', type: 'CAMPAIGN_APPROVAL', title: 'Approve GoClear funding-readiness campaign and test communication', status: 'DRAFT_REVIEW_REQUIRED', why_human_review_required: 'Ray approval is required before social publication or email delivery.', what_ray_is_deciding: 'Approve the bounded campaign package, reject it, or request changes from the assigned departments.', artifact_refs: ['admin_approval_1465222613e04f1fa8a10e8e8a83674c', 'research_package_c12e6faaab884d5aa34c76641bc5c33c', 'alpha_receipt_481fd7fad572404da766712c0b2fc9d6'], risk: 'HIGH', external_action_if_approved: 'Resume the existing cycle; social and email remain separately receipt-gated.' }
...
export default function LiveCompanyIntelligence({ surface = null }) {
  const fallbackState = normalizeAdminState({
    ...
    ray_decisions: { ...(snapshot.ray_decisions || {}), count: reviewQueue.length + 1, items: [existingCampaignApproval, ...reviewQueue] },
  ...
  return <main className="live-command-page">
    ...
    <div className="live-command-main">
      ...
      <div className="live-command-content">
        ...
        <div className="live-command-metrics">{metrics.map(([icon, label, value, note, tone]) => <CommandMetric key={label} icon={icon} label={label} value={value} note={note} tone={tone} />)}</div><GoclearBetaProjectionPanel /><MarketingCampaignCertificationPanel />
