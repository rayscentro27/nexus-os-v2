...
const iconMap = {
  ...
  campaigns: Target, decisions: CheckSquare, trading: TrendingUp,
}
...
const canonicalNav = [
  ...
  ['campaigns', 'Campaigns', 'campaigns'], ['decisions', 'Decisions', 'decisions'], ['team', 'Team', 'team'],
...
function ProjectSurface({ state }) {
  ...
  return <div className="live-surface-page"><SurfaceHeader eyebrow="WORK / PROJECT PORTFOLIO" title="Projects" description="Turn ideas into real progress." quote="Progress you can see." /><div className="live-surface-toolbar"><div className="live-surface-tabs">{['All', 'Active', 'Planning', 'Completed'].map((tab, i) => <button className={i === 0 ? 'active' : ''} key={tab}>{tab}</button>)}</div><div className="live-surface-toolbar-actions"><label><Search size={14} /><input placeholder="Search projects" /></label><button className="live-surface-primary"><FolderPlus size={14} /> New Project</button></div></div><div className="live-project-grid">{projects.map((project, index) => <article className={`live-project-card accent-${['green', 'blue', 'purple', 'amber'][index % 4]}`} key={project.name}><div className="live-project-card-top"><span>{project.status || 'STATUS UNAVAILABLE'}</span><Target size={16} /></div><h3>{project.name}</h3><p>{project.next || 'Project detail is not currently available.'}</p><div className="live-project-progress"><div><span>Progress</span><b>—</b></div><i><em style={{ width: '0%' }} /></i></div><footer><span>Team · {project.department || 'Unavailable'}</span><span>Due date · Unavailable</span></footer></article>)}</div>{!projects.length && <div className="live-honest-empty"><FolderKanban size={20} /><strong>No project records currently available</strong><span>Live project data will appear here when the governed source returns it.</span></div>}<div className="live-project-overview"><CommandPanel title="Project Health Overview" eyebrow="PORTFOLIO HEALTH"><div className="live-project-health"><div className="live-health-donut"><strong>{projects.length}</strong><small>Projects</small></div><div className="live-health-legend"><span><i className="green" />On Track <b>Unavailable</b></span><span><i className="amber" />At Risk <b>Unavailable</b></span><span><i className="red" />Delayed <b>Unavailable</b></span><span><i className="purple" />Completed <b>{state.campaigns?.completed ?? '—'}</b></span></div><div className="live-health-kpis"><SurfaceStat label="Avg Progress" value="—" /><SurfaceStat label="Active Projects" value={state.campaigns?.active} tone="teal" /><SurfaceStat label="Completed" value={state.campaigns?.completed} tone="purple" /></div></div></CommandPanel></div></div>
}

function ResearchSurface({ state }) {
  const research = state.research || {}; const findings = state.recent_findings || [];
  return <div className="live-surface-page"><SurfaceHeader eyebrow="INTELLIGENCE / RESEARCH REVIEW" title="Research" description="See what Nexus is studying, what it found, and what still needs proof." quote="Evidence before action." /><div className="live-research-status"><SurfaceStat label="Current objective" value={research.current_objective || state.next_machine_action || 'Unavailable'} /><SurfaceStat label="Current lane" value={research.current_lane || 'Unavailable'} tone="teal" /><SurfaceStat label="Process health" value={research.process_health || research.health || 'Unavailable'} tone="green" /><SurfaceStat label="Productivity health" value={research.productivity_health || 'Unavailable'} tone="purple" /><SurfaceStat label="Last successful wake" value={research.last_successful_wake || research.last_real_run || 'Unavailable'} tone="amber" /><SurfaceStat label="Next wake" value={research.next_wake || state.next_wake_at || 'Scheduler-managed'} /></div><div className="live-review-grid"><CommandPanel title="Recent Findings" eyebrow="SOURCE-BACKED"><div>{findings.map((item, i) => <DetailRow key={item.title || i} icon="research" title={item.title || 'Finding'} detail={`${item.lane || 'Lane unavailable'} · ${item.owner || 'Owner unavailable'}`} meta={item.state || 'STATUS UNAVAILABLE'} tone={i % 2 ? 'teal' : 'cyan'} />)}</div>{!findings.length && <div className="live-honest-empty">No findings currently available.</div>}</CommandPanel><CommandPanel title="Validation status" eyebrow="EVIDENCE"><div className="live-review-stat-list"><SurfaceStat label="Validated claims" value={research.validated_claims ?? state.youtube?.claims_validated} tone="green" /><SurfaceStat label="Partially supported" value={research.partially_supported_claims ?? '—'} tone="amber" /><SurfaceStat label="Unverified" value={research.unverified_claims ?? '—'} tone="purple" /><SurfaceStat label="Follow-up queue" value={research.follow_up_queue_depth ?? '—'} tone="orange" /></div></CommandPanel></div><div className="live-review-grid"><CommandPanel title="What needs review" eyebrow="FOLLOW-UP"><div className="live-detail-list"><p><strong>Sources</strong><span>{research.sources_checked ?? '—'} source records checked</span></p><p><strong>Opportunities created</strong><span>{state.opportunities?.active ?? '—'} canonical records; overnight new count unavailable</span></p><p><strong>Incidents</strong><span>{research.incidents ?? '—'} current Research incidents</span></p></div></CommandPanel><CommandPanel title="Research continuation" eyebrow="NEXT ACTION"><p className="live-surface-copy">{state.next_machine_action || 'No next Research action is currently reported.'}</p><a className="live-surface-link" href="#/knowledge">Open Knowledge <ArrowUpRight size={13} /></a></CommandPanel></div></div>
}
...
function DecisionsSurface({ state }) {
  ...
  return <div className="live-surface-page"><SurfaceHeader eyebrow="GOVERNANCE / HUMAN GATE" title="Ray Decision Queue" description="Review decisions that require Ray before Nexus can continue." quote="Clarity at the right moment." /><div className="live-surface-toolbar"><div className="live-surface-tabs">{['Needs Review', 'Approved', 'Rejected', 'Deferred', 'Completed'].map((tab, i) => <button className={i === 0 ? 'active' : ''} key={tab}>{tab}</button>)}</div></div><CommandPanel title={`${state.ray_decisions?.count ?? items.length} decision(s) require review`} eyebrow="CURRENT QUEUE"><div>{items.map((item, i) => <DetailRow key={item.id || item.title || i} icon="decisions" title={item.title || item} detail={item.reason || 'Supporting evidence unavailable'} meta={item.status || 'OPEN'} tone="amber" />)}</div>{!items.length && <div className="live-honest-empty"><CheckSquare size={20} /><strong>No decisions waiting</strong><span>Routine machine work is continuing; no Ray action is currently reported.</span></div>}</CommandPanel></div>
}

function AiCommandSurface({ state }) {
  return <div className="live-surface-page live-ai-page"><SurfaceHeader eyebrow="COMMAND / NOVA" title="AI Command" description="Your intelligent assistant for a clearer tomorrow." quote="Turn your questions into progress." /><div className="live-ai-modes"><div>{['Chat', 'Analyze', 'Create', 'Automate'].map((mode, i) => <button className={i === 0 ? 'active' : ''} key={mode}>{mode}</button>)}</div><span><Sparkles size={13} /> Powered by Nexus AI</span></div><div className="live-ai-workspace"><div className="live-ai-orb"><div className="live-ai-orb-core">N</div><i /><i /><i /></div><div className="live-ai-chat"><NexusAgentConversation agent="nova" context="Current Admin surface: AI Command" /></div></div><div className="live-ai-actions">{['Analyze my business data', 'Create a project plan', 'Write marketing content', 'Build an automation', 'Summarize recent activity', 'Give me insights'].map((action, i) => <button key={action}><span>{i + 1}</span>{action}<ArrowUpRight size={13} /></button>)}</div><div className="live-ai-lower"><CommandPanel title="Recent AI Conversations" eyebrow="NOVA"><div className="live-honest-empty">Saved Nova conversations appear here.</div></CommandPanel><CommandPanel title="Suggested Automations" eyebrow="NEXUS AI"><div className="live-honest-empty">No suggested automations currently available.</div></CommandPanel><CommandPanel title="Quick Outputs" eyebrow="OUTPUTS"><div className="live-honest-empty">No quick outputs currently available.</div></CommandPanel></div></div>
}
  ...
  return <main className="live-command-page">
    ...
    <div className="live-command-main">
      ...
      <div className="live-command-content">
        {activeSurface !== 'live-intelligence' ? <div className="live-command-surface">{activeSurface === 'ai-command' ? <AiCommandSurface state={s} /> : activeSurface === 'projects' ? <ProjectSurface state={s} /> : activeSurface === 'research' ? <ResearchSurface state={s} /> : activeSurface === 'campaigns' ? <CampaignsSurface state={s} /> : activeSurface === 'decisions' ? <DecisionsSurface state={s} /> : activeSurface === 'trading' ? <TradingLabPanel /> : <AdminSurfacePage surface={activeSurface} />}</div> : <>
