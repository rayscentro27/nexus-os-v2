...
function AiCommandSurface({ state }) {
  ...
  return <div className="live-surface-page live-ai-page"><SurfaceHeader eyebrow="COMMAND / NOVA" title="AI Command" description="Your intelligent assistant for a clearer tomorrow." quote="Turn your questions into progress." /><div className="live-ai-modes"><div>{['Chat', 'Analyze', 'Create', 'Automate'].map(item => <button type="button" className={mode === item ? 'active' : ''} onClick={() => setMode(item)} key={item}>{item}</button>)}</div><span><Sparkles size={13} /> Powered by Nexus AI · {mode}</span></div><div className="live-ai-workspace"><div className="live-ai-orb"><div className="live-ai-orb-core">N</div><i /><i /><i /></div><div className="live-ai-chat"><NexusAgentConversation agent="nova" context={`Current Admin surface: AI Command · ${mode} mode`} /></div></div><div className="live-ai-actions">{['Analyze my business data', 'Create a project plan', 'Write marketing content', 'Build an automation', 'Summarize recent activity', 'Give me insights'].map((action, i) => <button type="button" disabled title="NOT_YET_AVAILABLE: suggested-prompt insertion is not wired" key={action}><span>{i + 1}</span>{action}<ArrowUpRight size={13} /></button>)}</div><div className="live-ai-lower"><CommandPanel title="Recent AI Conversations" eyebrow="NOVA"><div className="live-honest-empty">Saved Nova conversations appear here.</div></CommandPanel><CommandPanel title="Suggested Automations" eyebrow="NEXUS AI"><div className="live-honest-empty">No suggested automations currently available.</div></CommandPanel><CommandPanel title="Quick Outputs" eyebrow="OUTPUTS"><div className="live-honest-empty">No quick outputs currently available.</div></CommandPanel></div></div>
}
