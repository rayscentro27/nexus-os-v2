...
function SurfaceBody({ surface, state }) {
  ...
  return <>
    ...
    {surface === 'ai-command' && <SurfaceCard title="Open a governed command room" eyebrow="COMMAND ENTRY"><div className="admin-surface-command-entry"><MessageSquare size={18} /><div><strong>Use the canonical Nova conversation surface</strong><p>Commands remain governed by the current agent, approval, and receipt contracts. This page does not invent a second execution path.</p></div><a href="#/ai-command">Open AI Command <ArrowRight size={13} /></a></div></SurfaceCard>}
    ...
    <div className="admin-surface-grid secondary">
      ...
      <SurfaceCard title="Quick destinations" eyebrow="NAVIGATION"><div className="admin-surface-destination-grid">{[['Dashboard', 'live-intelligence', Activity], ['Research', 'research', Radar], ['Tasks', 'tasks', ListTodo], ['Knowledge', 'knowledge', BookOpen], ['Research opportunities', 'research', Target], ['Support', 'support', LifeBuoy]].map(([label, href, Icon]) => <a href={`#/${href}`} key={label}><Icon size={15} /><span>{label}</span><ArrowRight size={12} /></a>)}</div></SurfaceCard>
    </div>
  </>
}
