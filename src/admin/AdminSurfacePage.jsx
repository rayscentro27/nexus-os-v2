...
const SURFACES = {
  ...
  team: { title: 'Team', eyebrow: 'OPERATIONS / OWNERSHIP', icon: Users, description: 'Departments and runtime identities with current ownership, availability, and work context.' },
...
function SurfaceBody({ surface, state }) {
  ...
  const metricSet = {
    ...
    team: [['Departments', departments.length, 'registered views', 'cyan'], ['Active', departments.filter(d => /active/i.test(d.status || '')).length, 'currently operating', 'green'], ['Ready', departments.filter(d => /ready/i.test(d.status || '')).length, 'awaiting work', 'amber'], ['Legitimately idle', departments.filter(d => /idle/i.test(d.status || '')).length, 'no current queue', 'purple']],
  ...
  const rows = surface === 'projects' ? campaigns : surface === 'knowledge' ? findings : surface === 'team' ? departments : surface === 'automation' ? workers : surface === 'ai-command' ? workers : []
  return <>
