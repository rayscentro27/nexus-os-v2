/** Capture Queue — reads task_requests (the capture/intake requests). Read-only. Shows queue +
 *  worker status: queued/running/captured/done/failed, and links to the captured source/review.
 *  Safe items wait for the local runner; review-required items also appear in Approvals. */
import { listTable, type Row } from '../../services/db';
import { useData } from '../ui';

const CAPTURE_TYPES = new Set(['youtube_capture_request', 'source_capture_request', 'research_source_intake']);

function statusView(r: Row): { label: string; cls: string; note: string } {
  const p = (r.payload ?? {}) as Record<string, unknown>;
  const cs = String(p.capture_status ?? '');
  const approvalRequired = p.approval_required === true;
  if (cs === 'captured' || r.status === 'done') return { label: 'Captured', cls: 'ok', note: 'Captured — ready for review.' };
  if (cs === 'failed' || r.status === 'failed') return { label: 'Failed', cls: 'warnb', note: 'Capture failed — see result summary.' };
  if (cs === 'running' || r.status === 'running') return { label: 'Running', cls: 'infob', note: 'Local runner is capturing…' };
  if (approvalRequired) return { label: 'Approval required', cls: 'warnb', note: `Needs Ray review in Approvals${p.review_trigger ? ` (${p.review_trigger})` : ''}.` };
  return { label: 'Safe capture', cls: 'ok', note: 'Waiting for local runner.' };
}

export function PendingCaptureRequests({ refresh }: { refresh: number }) {
  const { data } = useData<Row[]>(
    () => listTable('task_requests', { order: 'created_at', limit: 20 }),
    [], refresh);
  const rows = data.filter((r) => CAPTURE_TYPES.has(r.task_type));

  return (
    <div className="nx-glass">
      <div className="nx-between" style={{ marginBottom: 8 }}>
        <h3 style={{ margin: 0 }}>Capture Queue <span className="nx-muted" style={{ fontSize: 12 }}>({rows.length})</span></h3>
      </div>
      <div className="nx-muted" style={{ fontSize: 11, marginBottom: 6 }}>Safe capture can queue automatically. Risky actions still require Ray approval.</div>
      {rows.length === 0 ? (
        <div className="nx-muted" style={{ fontSize: 12 }}>Nothing queued. Submit a YouTube URL above — safe sources queue automatically (no approval).</div>
      ) : rows.map((r) => {
        const p = (r.payload ?? {}) as Record<string, unknown>;
        const sv = statusView(r);
        return (
          <div key={r.id} className="nx-soft" style={{ padding: 10, marginBottom: 6, fontSize: 12 }}>
            <div className="nx-between">
              <b className="nx-truncate" style={{ maxWidth: 200, display: 'inline-block' }}>{String(p.title || p.source_url || r.task_type)}</b>
              <span className={`nx-badge ${sv.cls}`}>{sv.label}</span>
            </div>
            <div className="nx-muted" style={{ marginTop: 4, wordBreak: 'break-all' }}>{String(p.source_url ?? '')}</div>
            <div className={sv.cls === 'warnb' ? 'nx-amber' : sv.cls === 'ok' ? 'nx-green' : 'nx-blue'} style={{ marginTop: 4 }}>{sv.note}</div>
            {r.result_summary && <div className="nx-muted" style={{ marginTop: 4 }}>result: {r.result_summary}</div>}
            {typeof p.research_source_id === 'string' && <div className="nx-muted" style={{ marginTop: 2 }}>source {String(p.research_source_id).slice(0, 8)}…</div>}
            {typeof p.capture_command_preview === 'string' && (
              <details className="nx-collapse" style={{ marginTop: 4 }}>
                <summary className="nx-muted">command preview</summary>
                <code style={{ fontSize: 11, wordBreak: 'break-all' }}>{p.capture_command_preview as string}</code>
              </details>
            )}
            <div className="nx-muted" style={{ marginTop: 4 }}>queue {r.id.slice(0, 8)}… · {r.created_at ? new Date(r.created_at).toLocaleString() : ''}</div>
          </div>
        );
      })}
    </div>
  );
}
