...
export function ClientMetricCard({ icon: Icon, label, value, note, tone = 'cyan' }) {
  return <section className="client-card client-metric-card"><div className={`client-metric-icon ${tone}`}><Icon size={24} /></div><span>{label}</span><strong>{value}</strong><small>{note}</small></section>
}

export function ClientScoreCard({ title, value, status, text, label = '/100' }) {
  return <section className="client-card client-score-card"><h3>{title}</h3><div><ClientProgressRing value={value} label={label} /><div className="client-score-copy"><strong>{status}</strong><p>{text}</p></div></div></section>
}
...
export function ClientActionList({ rows }) {
  return <div className="client-action-list">{rows.map((row, index) => {
    ...
    return <article key={title}><span>{index + 1}</span><div><strong>{title}</strong><small>{status}</small></div><ChevronRight size={16} /></article>
  })}</div>
}
