...
function DashboardSurface({ readiness, journey, navigate }) {
  return <section className="wc-card wc-guidedDashboard" data-testid="guided-dashboard">
    ...
    <div className="wc-guidedColumns"><div><b>Recent activity</b>{(readiness.readinessHistory.length ? readiness.readinessHistory : [{ label: 'Journey state calculated from persisted profile and evidence', status: 'observed', date: '' }]).slice(0, 3).map(item => <p key={`${item.label}-${item.date}`}>• {item.label} · {item.status}</p>)}</div><div><b>Clyde guidance</b><p>Clyde separates Nexus-observed facts, client-provided information, uploaded evidence, uncertainty, and recommendations. Outcomes are not guaranteed.</p></div><div><b>Review eligibility</b><p>{readiness.reviewEligible ? 'Eligible to request a controlled review.' : 'Not yet eligible. Complete outstanding items and wait for processing or specialist review.'}</p></div></div>
  </section>
}
...
export default function GuidedClientJourneySurface({ routeKey, readiness, journey, existingDocuments, openUploadPanel, onUploaded, navigate }) {
  ...
  if (routeKey === 'credit') return <div className="wc-guidedStageStack"><StageCard stage={readiness.stages.credit} existingDocuments={existingDocuments} openUploadPanel={openUploadPanel} onUploaded={onUploaded} navigate={navigate} /><div className="wc-card wc-guidedCreditNote"><h2>Credit stage guidance</h2><p>Credit Profile and Credit Utilization are one guided stage. Utilization, discrepancies, strategies, evidence, and safe drafts stay connected here.</p><div className="wc-guidedMetricGrid"><div><small>Done</small><b>{journey?.completedCount ?? readiness.completedRequirements.length}</b></div><div><small>Remaining</small><b>{Math.max(0, (journey?.totalCount ?? (readiness.completedRequirements.length + readiness.outstandingRequirements.length)) - (journey?.completedCount ?? readiness.completedRequirements.length))}</b></div></div><button onClick={() => navigate('/client/dispute-review')}>Open safe draft review</button></div></div>
