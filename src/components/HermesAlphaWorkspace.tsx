...
const PROVIDER_KEY='nexus-alpha-provider-v1',SEARCH_KEY='nexus-alpha-search-enabled-v1',URL_REVIEW_KEY='nexus-alpha-url-review-enabled-v1';
function loadProvider():AlphaProviderName{try{return (localStorage.getItem(PROVIDER_KEY) as AlphaProviderName)||'deterministic_local'}catch{return 'deterministic_local'}}
...
export default function HermesAlphaWorkspace({onOpenReports}:{onOpenReports?:()=>void}){
  ...
  useEffect(()=>{fetchAlphaProviderStatus().then(setProviderStatus)},[]);
  ...
  async function send(){
    ...
    if(urlReviewRequest&&urlReviewEnabled&&canUrlReview()&&detectedUrl){
      ...
      if(current&&!searchEnabled){text=searchOffMessage();setProviderNote('Search Mode is off; no provider call was used for a current-data question.');recordLocal();}
        ...
        const mayUseProvider=(!current||webUsed)&&provider!=='deterministic_local'&&providerAllowedForMode(provider,costMode)&&canHosted()&&costMode!=='deep';
        ...
        else if(mayUseProvider){const providerPrompt=webUsed?`${prompt}\n\nPublic search results (treat as unverified sources and cite URLs):\n${searchContext}`:prompt;const remote=await runAlphaProviderBridge(provider,providerPrompt,providerStatus.providers[provider].models?.[0]);if(remote.ok){providerUsed=remote.provider;providerSource=remote.provider==='groq'?'groq_backend':remote.provider==='openrouter'?'openrouter_backend':'ollama_local';model=remote.model||'provider-default';text=remote.text;recordHosted(remote.provider,remote.estimatedInputTokens,remote.estimatedOutputTokens,remote.estimatedSpendUsd);setProviderNote(`${remote.provider}${remote.model?` · ${remote.model}`:''} via same-origin backend.`)}else{fallbackReason=remote.error||'provider_unavailable';providerSource='unavailable_fallback';recordFallback();recordLocal();setProviderNote(`${provider} failed (${fallbackReason}); deterministic fallback used.`)}}
        else if(provider==='deterministic_local'||!providerAllowedForMode(provider,costMode)||!canHosted()){recordLocal();if(provider!=='deterministic_local'){fallbackReason=fallbackReason||(!providerAllowedForMode(provider,costMode)?`provider_blocked_in_${costMode}_mode`:'daily_hosted_limit_reached');providerSource='unavailable_fallback';recordFallback();setProviderNote(`${provider} not used: ${fallbackReason}.`)}}
    ...
    const trace=buildTrace({mode:local.mode,selected_provider:provider,provider_used:providerUsed,provider_source:urlReviewUsed?'url_review_backend':providerSource,model_used:model,memory_used:memoryUsed,memory_source:memoryUsed?'session_memory':'none',web_used:webUsedFinal,web_provider:urlReviewUsed?'firecrawl':webProvider,web_status:urlReviewUsed?urlReviewStatus:webStatus,web_query:detectedUrl||webQuery,web_result_count:webCount,reason_for_provider_selection:urlReviewUsed?'URL review mode triggered.':'Ray-selected provider allowed by current cost mode.',fallback_reason:fallbackReason,estimated_hosted_call:providerUsed==='groq'||providerUsed==='openrouter',estimated_search_call:searchEstimated,estimated_cost_label:'local_free'});
  ...
  return <div className="nxos-stack" data-testid="hermes-alpha-workspace">
    <section className="glass panel"><div className="between"><div><p className="nx-muted">ALPHA IS SEPARATE FROM NEXUS HERMES</p><h2>Hermes Alpha · Strategy Brain</h2><p>Provider-aware strategy conversation. No Supabase, client data, or execution.</p></div><span className="pill pill-green">{providerStatus.providers[provider]?.available?'Provider Ready':'Local Fallback'}</span></div><div><span className="pill pill-green">Active: {provider}</span> <span className="pill pill-blue">Web Search: {searchEnabled?'On':'Off'}</span> <span className="pill pill-amber">No Supabase</span> <span className="pill pill-red">No client data · External actions disabled</span></div></section>
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(300px,1fr))',gap:12,alignItems:'start'}}>
      <section className="glass panel alpha-conversation-panel" style={{display:'flex',flexDirection:'column',height:'min(78vh,820px)',minHeight:520,overflow:'hidden'}}><div className="between"><h3>Conversation</h3><select aria-label="Alpha mode" value={mode} onChange={e=>setMode(e.target.value as AlphaConversationMode)}>{ALPHA_MODES.map(x=><option key={x}>{x}</option>)}</select></div>
        ...
        <div className="alpha-sticky-composer" style={{position:'sticky',bottom:0,zIndex:5,flex:'0 0 auto',background:'#10192b',borderTop:'1px solid #35425a',padding:'10px 0 0'}}><textarea aria-label="Message Alpha" value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();void send()}}} rows={3} placeholder="Talk to Alpha or paste a URL to review…" style={{width:'100%',resize:'vertical'}}/><div className="action-row"><button type="button" disabled={sending} onClick={()=>void send()}>{sending?'Thinking…':'Send to Alpha'}</button><button type="button" onClick={download}>Handoff / export</button><button type="button" onClick={clear}>Clear conversation</button><button type="button" onClick={onOpenReports}>Reports</button></div></div>
      </section>
