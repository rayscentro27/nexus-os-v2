...
export default function HermesAlphaWorkspace({onOpenReports}:{onOpenReports?:()=>void}){
  ...
  return <div className="nxos-stack" data-testid="hermes-alpha-workspace">
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(300px,1fr))',gap:12,alignItems:'start'}}>
      <section className="glass panel alpha-conversation-panel" data-testid="alpha-conversation-panel" style={{display:'grid',gridTemplateRows:'auto minmax(0,1fr) auto',height:'clamp(360px, calc(100vh - 500px), 640px)',minHeight:0,overflow:'hidden',paddingBottom:12}}><div><div className="between"><h3>Conversation</h3><select aria-label="Alpha mode" value={mode} onChange={e=>setMode(e.target.value as AlphaConversationMode)}>{ALPHA_MODES.map(x=><option key={x}>{x}</option>)}</select></div><div className="nx-soft" style={{padding:10,margin:'8px 0'}}>Ask Alpha about a strategy, opportunity, app, voice agent, affiliate idea, or paste a URL to review.</div><small data-testid="alpha-composer-status" style={{display:'block',color:'#93c5fd',marginBottom:6}}>Composer: mounted · {sending?'sending':'enabled'}</small></div>
        <div data-testid="alpha-transcript" aria-label="Alpha conversation" style={{minHeight:0,overflowY:'auto',display:'grid',alignContent:'start',gap:8,padding:'8px 2px'}}>{messages.map(m=><article key={m.id} className="nx-soft" style={{padding:12,marginLeft:m.role==='ray'?'12%':0,borderColor:m.role==='ray'?'#4372b8':'#7857d8'}}><strong>{m.role==='ray'?'Ray':'Hermes Alpha'} · {m.mode}</strong><p style={{whiteSpace:'pre-wrap'}}>{m.text}</p>{m.role==='alpha'&&m.trace&&<details style={{fontSize:11,marginTop:5,borderTop:'1px solid #333',paddingTop:5}}><summary style={{cursor:'pointer'}}>{traceSummary(m.trace)}</summary><pre style={{whiteSpace:'pre-wrap',fontSize:10}}>{JSON.stringify(m.trace,null,2)}</pre></details>}</article>)}</div>
        <form className="alpha-sticky-composer" data-testid="alpha-composer" onSubmit={e=>{e.preventDefault();void send()}} style={{position:'relative',zIndex:20,flex:'0 0 auto',background:'#10192b',borderTop:'1px solid #35425a',padding:'10px 0 0',display:'grid',gap:8}}><textarea data-testid="alpha-composer-input" aria-label="Message Alpha" value={input} disabled={sending} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();void send()}}} rows={3} placeholder="Ask Hermes Alpha..." style={{width:'100%',minHeight:84,resize:'none',display:'block'}}/><div className="action-row"><button data-testid="alpha-composer-send" type="submit" disabled={sending} style={{minHeight:36}}>{sending?'Thinking…':'Send to Alpha'}</button><button type="button" onClick={download}>Handoff / export</button><button type="button" onClick={clear}>Clear conversation</button><button type="button" onClick={onOpenReports}>Reports</button></div></form>
      </section>
