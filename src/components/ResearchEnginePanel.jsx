...
function ResearchCandidateDrawer({ candidate, onClose, onAskHermes, sourceType }) {
...
export default function ResearchEnginePanel({ onAskHermes }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true)
      const result = await loadSection('research_engine', researchCandidates)
      if (!cancelled) {
  ...
  const items = sectionResult ? sectionResult.records : researchCandidates

  function getEffectiveStatus(c) {
    return statusOverrides[c.id] || c.status
  }

  const lanes = [...new Set(items.map(c => c.lane))].sort()
  const statuses = [...new Set(items.map(c => c.status))].sort()
    ...
    <div className="nxos-stack">
      ...
      <div className="nxos-toolbar">
        <div className="nxos-chiprow">
          <button type="button" className={laneFilter === 'all' ? 'active' : ''} onClick={() => setLaneFilter('all')}>All lanes ({items.length})</button>
          {lanes.map(l => (
            <button key={l} type="button" className={laneFilter === l ? 'active' : ''} onClick={() => setLaneFilter(l)} style={{ textTransform: 'capitalize' }}>
              {l.replace(/_/g, ' ')} ({items.filter(c => c.lane === l).length})
            </button>
          ))}
        </div>
      </div>
      ...
      <ResearchCandidateDrawer candidate={selected} onClose={() => setSelected(null)} onAskHermes={onAskHermes} sourceType={sectionResult ? sectionResult.sourceType : 'static_fallback'} />
    </div>
  )
}
