...
const endpoint = import.meta.env.VITE_NEXUS_NOVA_LOCAL_ENDPOINT || 'http://127.0.0.1:8790/v1/nova/chat'
...
export default function NovaWorkspace() {
  ...
  async function send(text = input) {
    ...
    try {
      ...
      const response = await fetch(endpoint, { method: 'POST', credentials: 'same-origin', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${accessToken}`, 'X-Nexus-Nova-Session': 'admin-browser' }, body: JSON.stringify({ message: clean, conversation_id: 'admin-browser', channel: 'admin_browser', recent_history: messages.slice(-12).map(item => ({ role: item.role === 'ray' ? 'user' : 'assistant', content: item.text })) }) })
