...
function OpportunityDetailDrawer({ opportunity, onClose, onAskHermes }) {
...
export default function BusinessOpportunitiesPanel({ onAskHermes }) {
  ...
  const filtered = businessOpportunities.filter(opp => {
  ...
  const counts = {
    all: businessOpportunities.length,
    scored: businessOpportunities.filter(o => getEffectiveStatus(o) === 'scored').length,
    approved: businessOpportunities.filter(o => getEffectiveStatus(o) === 'approved').length,
    held: businessOpportunities.filter(o => getEffectiveStatus(o) === 'held').length,
    rejected: businessOpportunities.filter(o => getEffectiveStatus(o) === 'rejected').length,
  }
  ...
  return (
    <div className="nxos-stack">
      ...
      <OpportunityDetailDrawer opportunity={selected} onClose={() => setSelected(null)} onAskHermes={onAskHermes} />
    </div>
  )
}
