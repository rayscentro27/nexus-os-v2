...
function CreditPanel({ navigate, existingDocuments, onUploaded, openUploadPanel }) {
...
function RepairPanel({ scores, navigate, existingDocuments, onUploaded, openUploadPanel }) {
  ...
  return <section className="wc-panel wc-panel-repair"><Hero /><div className="wc-card wc-repairJourney"><div className="wc-repairLine">{['Profile Complete', 'Upload Credit Report', 'Specialist Review', 'Dispute Items', 'Draft Letters', 'Approve & Send', 'Track Results'].map((title, i) => <div key={title}><span className={`wc-stepDot ${completed.includes(stepKeys[i]) ? 'done' : current === stepKeys[i] ? 'active' : ''}`}>{i + 1}</span><div className="wc-softIcon">{['👤', '☁', '🔍', '⚖', '✎', '✈', '📊'][i]}</div><b>{title}</b><p>{completed.includes(stepKeys[i]) ? 'Complete' : current === stepKeys[i] ? 'In Progress' : 'Upcoming'}</p></div>)}</div></div><UploadLane title="Upload Credit Repair Evidence" description="Upload one credit report or support document at a time. Clyde will organize it for GoClear review." onUpload={() => openUploadPanel({ track: 'credit_repair', pageContext: 'credit_repair', suggestedCategory: 'credit_report', title: 'Upload Credit Repair Evidence', description: 'Upload one credit repair document for Pending GoClear Review.' })} onVault={() => navigate('/client/documents')} /><div className="wc-repairMid"><div className="wc-card"><SectionHead title="Your Next Actions" action="View all" /><div className="wc-actionRow three"><ActionCard icon="☁" title="Upload your credit report" text="This helps us analyze your file." button="Upload Now" onClick={() => openUploadPanel({ track: 'credit_repair', pageContext: 'credit_repair', suggestedCategory: 'credit_report', title: 'Upload Credit Report', description: 'Upload one current credit report for Pending GoClear Review.' })} /><ActionCard icon="👥" title="Answer a few questions" text="Help us understand your goals." button="Continue Profile" onClick={() => navigate('/client/profile')} /><ActionCard icon="➕" title="Review dispute letters" text={`${journey?.letters?.length || 0} letter(s) in workflow.`} button="Review" onClick={() => navigate('/client/dispute-review')} /></div><div className="wc-inlineRequirementGrid"><InlineDocumentRequirement title="Credit Report" description="Upload a current report for the repair workflow." category="credit_report" requirementKey="credit_report" fromPage="credit-repair" impactLabel="Required" existingDocuments={existingDocuments} onUploaded={onUploaded} onOpenUpload={({ title, category, fromPage, description }) => openUploadPanel({ track: 'credit_repair', pageContext: fromPage || 'credit_repair', suggestedCategory: category, title: `Upload ${title}`, description })} whyItMatters="Starts specialist review." /><InlineDocumentRequirement title="Supporting Dispute Documents" description="Upload evidence or supporting documents for dispute review." category="dispute_support" requirementKey="dispute_support" fromPage="credit-repair" impactLabel="Optional" required={false} existingDocuments={existingDocuments} onUploaded={onUploaded} onOpenUpload={({ title, category, fromPage, description }) => openUploadPanel({ track: 'credit_repair', pageContext: fromPage || 'credit_repair', suggestedCategory: category, title: `Upload ${title}`, description })} whyItMatters="Helps specialists draft accurate letters." /></div></div><div className="wc-card wc-progressBox"><h3>Progress Overview</h3><Donut value={percent} small tone="blue" /><p>{completed.length} completed · current step: {current.replaceAll('_', ' ')}</p></div></div><CreditRepairCaseEnginePanel existingDocuments={existingDocuments} onUploaded={onUploaded} navigate={navigate} openUploadPanel={openUploadPanel} /><div className="wc-repairBottom"><MiniCard icon="✉" title="Send From Home with DocuPost" tag="Approval gated" text="No letter is sent until specialist review and your explicit approval." button="Review gate" onClick={() => navigate('/client/dispute-review')} /><MiniCard icon="📝" title="Draft Letters" tag={`${journey?.letters?.length || 0} Ready`} text="Clyde and your specialist draft custom dispute letters." button="View drafts" onClick={() => navigate('/client/dispute-review')} /><MiniCard icon="⚑" title="Credit Monitoring Support" tag="Optional" text="Monitoring can support awareness, but upload/review drives this workflow." button="Resources" onClick={() => navigate('/client/resources?category=credit-monitoring')} /></div></section>
}
...
export default function WorldClassClientPortal({ path, onNavigate }) {
  ...
  const panel = showIcons ? <IconSystemPanel /> : {
    ...
    credit: <CreditPanel navigate={routeTo} existingDocuments={existingDocuments} onUploaded={refreshLiveData} openUploadPanel={openUploadPanel} />,
    ...
    funding: <FundingPanel scores={scores} navigate={routeTo} existingDocuments={existingDocuments} onUploaded={refreshLiveData} openUploadPanel={openUploadPanel} />,
