...
function ClydeChatDrawer({ open, onClose, navigate, pageTitle, clydeContext, onClydeAction }) {
  ...
  return <div className="wc-clydeOverlay" role="dialog" aria-modal="true" aria-label="Ask Hermes">
    <div className="wc-clydeDrawer">
      <div className="wc-clydeDrawerHead"><div className="wc-bot"><img src={clydeHermesAgent} alt="Clyde/Hermes advisor" /></div><div><h2>Ask Hermes</h2><p>{pageTitle} guidance</p></div><button onClick={onClose} aria-label="Close Hermes chat">×</button></div>
      ...
      {answer && <div className="wc-clydeAnswer"><b>Hermes</b><p>{answer}</p></div>}
      <label className="wc-clydeInput"><span>Message Hermes</span><input disabled placeholder="Live chat is coming soon. Use the suggested questions or request human review." /></label>
...
function ClydePanel({ navigate, onOpenChat, clydeContext, contextMessages = [], onClydeAction }) {
  ...
  return <aside className="wc-advisor"><div className="wc-advisorCard"><div className="wc-botTop"><div className="wc-bot"><img src={clydeHermesAgent} alt="Clyde/Hermes advisor" /></div><div><h3>Hermes Funding Readiness Guide</h3><div className="wc-online">● Online</div></div></div><p>{next.pageGoal} I’ll show what is missing, what GoClear is reviewing, and what needs your approval.</p><div className="wc-advisorBox"><h4>Persisted context</h4>{contextMessages.slice(0, 3).map(message => <p className="wc-clydeContextLine" key={message.id}><b>{message.source.replaceAll('_', ' ')}</b>: {message.text}</p>)}</div><div className="wc-advisorBox"><h4>Top Recommendations</h4>{recommendations.map((item, i) => <button className="wc-clydeItem" key={item.title} onClick={() => onClydeAction(item)}><ListItem tone={item.priority === 'high' ? 'orange' : item.priority === 'medium' ? 'blue' : 'green'} mark={String(i + 1)} title={item.title} text={item.reason} /></button>)}</div><div className="wc-advisorBox"><h4>Hermes Tip</h4><p>Upload one document and I’ll organize it for GoClear review. Documents is your vault, but you can upload from any workflow page.</p></div><div className="wc-suggestions"><button onClick={() => onClydeAction({ actionType: 'upload', label: 'Upload Document', uploadContext: { track: next.currentTrack } })}>Upload Document</button><button onClick={() => navigate('/client/funding-readiness')}>What does funding need?</button></div><button className="wc-chatBtn" onClick={onOpenChat}>Chat with Hermes</button></div></aside>
}
...
export default function WorldClassClientPortal({ path, onNavigate }) {
  ...
  const panel = showIcons ? <IconSystemPanel /> : {
    ...
    credit: <CreditPanel path={path} live={live} refreshLiveData={refreshLiveData} navigate={routeTo} existingDocuments={existingDocuments} onUploaded={refreshLiveData} openUploadPanel={openUploadPanel} />,
