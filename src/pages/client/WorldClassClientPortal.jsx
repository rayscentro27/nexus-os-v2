...
function HomePanel({ scores, live, profileComplete, navigate }) {
  ...
  return <section className="wc-panel wc-panel-home">
    ...
    <div className="wc-homeGrid">
      <div className="wc-card wc-recSteps"><SectionHead title="Next Best Action" action={nextActions[0]?.track} /><div className="wc-actionRow three">{nextActions.slice(0, 3).map((action, i) => <ActionCard key={action.title} icon={['☁', '♟', '⚑'][i]} title={action.title} text={action.description} button="Do This" onClick={() => navigate(action.route)} />)}</div></div>
    ...
    <div className="wc-card wc-uploadStrip"><div className="wc-uploadLead"><div className="wc-softIcon">☁</div><div><b>One place to move forward.</b><p>{uploaded} uploaded documents, {missing} missing. Clyde and GoClear use these to guide Credit Profile, Business Profile, and Business Funding.</p></div></div><div className="wc-uploadBtns"><button onClick={() => navigate('/client/documents')}>Upload Files</button><button onClick={() => navigate('/client/request-review')}>Request Review</button></div></div>
  </section>
}
...
function ClydePanel({ navigate, guidance, onOpenChat }) {
  ...
  return <aside className="wc-advisor"><div className="wc-advisorCard"><div className="wc-botTop"><div className="wc-bot">🤖</div><div><h3>Clyde Credit Specialist</h3><div className="wc-online">● Online</div></div></div><p>Start with your Credit Profile, Business Profile, or Business Funding track. I’ll show what is missing, what GoClear is reviewing, and what needs your approval.</p><div className="wc-advisorBox"><h4>Top Recommendations</h4>{recommendations.map((item, i) => <button className="wc-clydeItem" key={item.id || item.title} onClick={() => navigate(routeFromGuidance(item))}><ListItem tone={item.priority === 'high' ? 'orange' : item.priority === 'medium' ? 'blue' : 'green'} mark={item.priority === 'low' ? '✓' : String(i + 1)} title={item.title} text={item.description} /></button>)}</div><div className="wc-advisorBox"><h4>Clyde's Tip</h4><p>Your next step should either upload a missing document, choose a Credit Profile option, complete Business Profile setup, or request GoClear review.</p></div><div className="wc-suggestions"><button onClick={() => navigate('/client/credit-profile')}>Start with my Credit Profile</button><button onClick={() => navigate('/client/funding-readiness')}>What does funding need?</button></div><button className="wc-chatBtn" onClick={onOpenChat}>💬 Chat with Clyde</button></div></aside>
}
