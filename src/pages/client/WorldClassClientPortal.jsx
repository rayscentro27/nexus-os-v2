...
import { loadClientPortalLiveData, loadClientProfileIntake, saveClientProfileIntake, checkProfileIntakeComplete, saveCreditStrategyDecision, requestCreditStrategyTool } from '../../lib/clientPortalDataAdapter'
...
function CreditStrategyCards({ live, refreshLiveData, navigate }) {
  ...
  return <div className="wc-card"><SectionHead title="Clyde Strategy Cards" action={`${live.strategyRecommendations.length} automated option(s)`} /><p>Clyde states objective report differences and asks only for facts the report cannot provide. Outcomes are not guaranteed.</p>{live.strategyRecommendations.map(rec => { const p=rec.payload||{}; const d=p.discrepancy||{}; const tools=p.availableTools||[]; return <div key={rec.id} style={{border:'1px solid rgba(148,163,184,.25)',borderRadius:10,padding:12,marginTop:10}}><b>{p.primaryStrategy||rec.strategy_id}</b><p>{d.differenceSummary||p.rationale}</p><small>{Object.entries(d.bureauValues||{}).map(([b,v])=>`${b}: ${v}`).join(' · ')}</small><p><b>Why it matters:</b> {d.fundingImpact||'May affect Credit Profile and Tier 1/Tier 2 readiness.'}</p><p><b>Other approved options:</b> {(p.alternativeStrategies||[]).join(', ')||'Evidence organization and direct furnisher review when applicable.'}</p><p><b>Documents:</b> {(p.requiredEvidence||[]).join(', ')}</p><p><b>Limitations:</b> Nexus cannot guarantee correction, removal, score change, or funding approval.</p>{(p.clientConfirmationQuestions||[]).map(q=><p key={q}><b>Client fact needed:</b> {q}</p>)}<div className="wc-actionRow"><button onClick={()=>choose(rec,'selected')}>Use Recommended Strategy</button><button onClick={()=>choose(rec,'saved')}>Save for Later</button><button onClick={()=>choose(rec,'evidence_requested')}>Upload Evidence</button>{tools[0]&&<button onClick={()=>choose(rec,'draft_requested',tools[0])}>Prepare Draft</button>}<button onClick={()=>navigate('/client/request-review?topic=credit-strategy-exception')}>Request GoClear Exception Review</button></div>{decisions.get(rec.id)&&<small>Saved decision: {decisions.get(rec.id)}</small>}</div>})}{message&&<p>{message}</p>}</div>
}
