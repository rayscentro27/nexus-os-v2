...
import { loadClientPortalLiveData, loadClientProfileIntake, saveClientProfileIntake, checkProfileIntakeComplete, saveCreditStrategyDecision, requestCreditStrategyTool, saveVersionedStrategySelection, linkStrategyEvidence, generateClientStrategyDraft } from '../../lib/clientPortalDataAdapter'
...
import { CREDIT_STRATEGY_CATALOG } from '../../lib/creditStrategyCatalog'
import { CLIENT_FIVE_STAGES, buildFulfillmentView, billingVisibility, OUTSOURCED_FULFILLMENT_EXPLANATION } from '../../lib/clientStageModel'
import { trackEvent } from '../../lib/clientAnalytics'
import InlineDocumentUpload from '../../components/client/InlineDocumentUpload'
import ClientGuidedSurface from '../../components/client/GuidedClientJourneySurface'
import ClientRevenueServiceCard from '../../components/client/ClientRevenueServiceCard'
import { generateClydeMessages as generateContextClydeMessages } from '../../lib/clydeContextEngine'
import creditJourneyHero from '../../assets/nexus-3/credit/credit-journey-hero.png'
import creditRepairHero from '../../assets/nexus-3/credit/credit-repair-hero.png'
...
const heroAssets = {
  credit: creditJourneyHero,
  creditRepair: creditRepairBanner,
  business: businessJourneyHero,
  funding: fallbackHero,
}

const pageMeta = {
  '/client/dashboard': { key: 'home', title: 'Home' },
  '/client/profile': { key: 'profile', title: 'Business Profile' },
  '/client/credit-review': { key: 'credit', title: 'Credit Review' },
  '/client/credit-improvement': { key: 'improvement', title: 'Credit Improvement' },
  '/client/credit-profile': { key: 'credit', title: 'Credit Review' },
  '/client/credit-utilization': { key: 'credit', title: 'Credit Utilization' },
  '/client/account-details': { key: 'credit', title: 'Account Details' },
  '/client/documents': { key: 'documents', title: 'Documents' },
  '/client/business-foundation': { key: 'business', title: 'Business Foundation' },
  '/client/business-journey': { key: 'business', title: 'Business Foundation' },
  '/client/business-setup': { key: 'business', title: 'Business Foundation' },
  '/client/business-bankability': { key: 'business', title: 'Business Bankability' },
  '/client/business-credit': { key: 'business', title: 'Business Credit' },
  '/client/funding-readiness': { key: 'funding', title: 'Funding Readiness' },
  '/client/funding-access': { key: 'funding-access', title: 'Funding Access' },
  '/client/credit-repair-journey': { key: 'repair', title: 'Credit Repair' },
  '/client/dispute-review': { key: 'dispute', title: 'Safe Draft Review' },
  '/client/recommendations': { key: 'recommendations', title: 'Recommendations' },
  '/client/resources': { key: 'resources', title: 'Resources' },
  '/client/messages': { key: 'messages', title: 'Messages' },
  '/client/billing': { key: 'billing', title: 'Billing' },
...
const creditTabs = [
  ...
  ['/client/account-details', 'Account Details'],
  ['/client/dispute-review', 'Safe Draft Review'],
]

const businessTabs = [
  ['/client/business-foundation', 'Business Foundation'],
  ['/client/business-bankability', 'Business Bankability'],
]
...
function getCreditStrategyDisplayName(strategyId) {
  const found = CREDIT_STRATEGY_CATALOG.find(item => item.strategyId === strategyId || item.categories?.includes?.(strategyId))
  return found?.name || String(strategyId || 'Credit strategy').replaceAll('_', ' ')
}
...
function Hero({ variant = 'funding', eyebrow = 'Funding Readiness', title = 'Your Path to Funding', text = 'Build credit, business foundation, documents, and review readiness in one guided workspace.' }) {
  const src = heroAssets[variant] || fallbackHero
  return <div className={`wc-heroExact wc-n3Hero wc-n3Hero-${variant}`}><img src={src} alt={`${title} hero`} loading={variant === 'funding' ? 'lazy' : 'eager'} /><div className="wc-n3HeroShade" /><div className="wc-n3HeroContent"><span>{eyebrow}</span><h1>{title}</h1><p>{text}</p></div></div>
}

function SectionTabs({ tabs, currentPath, navigate }) {
  return <div className="wc-n3Tabs" role="tablist">{tabs.map(([route, label]) => <button key={route} type="button" role="tab" aria-selected={currentPath === route} className={currentPath === route ? 'active' : ''} onClick={() => navigate(route)}>{label}</button>)}</div>
}
...
function JourneyStagePanel({ readiness, journey, navigate }) {
  const statusFor = (stage) => {
    const key = stage.id === 'business_foundation' ? 'businessFoundation' : stage.id
    return journey?.stages?.[key]?.status || readiness?.stages?.credit?.status || 'not_started'
  }
  return <section className="wc-card wc-journeyPanel">
    <div className="wc-sectionHead"><div><h3>Your Journey — Five Stages</h3><p>Nexus guides you from credit review to verified, funding-ready outcomes.</p></div><span>Overall {readiness?.overallScore || 0}/100</span></div>
    <div className="wc-journeyStages">
      {CLIENT_FIVE_STAGES.map((stage, index) => {
        const status = String(statusFor(stage)).replaceAll('_', ' ')
        const active = readiness?.nextBestActionRoute === stage.path
        return <button key={stage.id} className={`wc-journeyStage ${active ? 'active' : ''}`} onClick={() => navigate(stage.path)}>
          <span className="wc-journeyIndex">{index + 1}</span>
          <div className="wc-journeyBody"><b>{stage.label}</b><p>{stage.description}</p><em>{status}</em></div>
          <span className="wc-journeyArrow">→</span>
        </button>
      })}
    </div>
  </section>
...
function NextStepCard({ readiness, navigate }) {
  const action = readiness?.nextBestAction || 'Complete your profile and upload the required documents.'
  const route = readiness?.nextBestActionRoute || '/client/credit-review'
  const example = String(action)
  return <div className="wc-card wc-nextStepCard">
    <div className="wc-nextLead"><div className="wc-softIcon">🎯</div><div><b>Your Next Step</b><h3>{example}</h3></div></div>
    <div className="wc-nextMeta"><span>Current status</span><b>{String(readiness?.state || 'insufficient information').replaceAll('_', ' ')}</b><p>{readiness?.primaryBlocker ? `Blocker: ${readiness.primaryBlocker}` : 'No blocker detected.'}</p></div>
    <button onClick={() => navigate(route)}>Continue →</button>
  </div>
}
...
function ProfilePanel({ navigate, onSaved, existingDocuments, openUploadPanel }) {
  return <section className="wc-panel wc-panel-profile"><Hero variant="business" eyebrow="Business Profile" title="Business Foundation Intake" text="Keep business identity, contact details, entity facts, and documents organized for funding readiness." /><div className="wc-card wc-simpleTrackHeader"><SectionHead title="Business Profile" action="Personal & business intake" /><p>Complete your business foundation so GoClear can prepare you for funding. This combines personal/contact basics, business setup, EIN/entity details, and required documents.</p></div><div className="wc-profileCards">
    {[
...
function CreditStrategyCards({ live, refreshLiveData, navigate }) {
  const [message, setMessage] = useState('')
  const [evidenceFor,setEvidenceFor]=useState(null)
  const [selectionIds,setSelectionIds]=useState({})
  const [answers,setAnswers]=useState({})
  const decisions = new Map((live?.strategyDecisions || []).map(d => [d.recommendation_id, d.decision]))
  const choose = async (rec, decision, toolType, factualAnswer='') => {
    const common = { recommendationId: rec.id, tenantId: live.resolvedTenantId, clientId: live.resolvedClientId }
    const result = await saveCreditStrategyDecision({ ...common, decision, newState: { strategyId: rec.strategy_id, strategyVersion: rec.strategy_version } })
    if (result.ok && toolType) await requestCreditStrategyTool({ ...common, toolType })
    if(result.ok&&['selected','saved','evidence_requested','declined','draft_requested'].includes(decision)){
      const p=rec.payload||{},d=p.discrepancy||{}
      const saved=await saveVersionedStrategySelection({tenantId:live.resolvedTenantId,clientId:live.resolvedClientId,reportId:rec.document_id||p.documentId||'unknown',canonicalAccountId:rec.canonical_account_id,discrepancyId:rec.discrepancy_id||d.discrepancyId,strategyId:rec.strategy_id,strategyVersion:rec.strategy_version,matchId:p.matchId,selectedOption:decision,status:decision==='evidence_requested'?'evidence_requested':decision==='draft_requested'?'awaiting_client_review':decision,clientAnswers:factualAnswer?{response:factualAnswer}:undefined})
      if(saved.ok){setSelectionIds(x=>({...x,[rec.id]:saved.selectionId})); if(decision==='draft_requested') await generateClientStrategyDraft({selectionId:saved.selectionId,tenantId:live.resolvedTenantId,clientId:live.resolvedClientId,strategyId:rec.strategy_id,strategyVersion:rec.strategy_version,outputType:toolType||'evidence_checklist',accountReference:d.accountReference||d.accountNumberMasked||'',detectedFacts:[d.differenceSummary||p.rationale||'Nexus detected a structured report difference.'],clientConfirmedFacts:factualAnswer?[factualAnswer]:[]})}
    }
    setMessage(result.ok ? 'Your choice was saved and will remain available across sessions.' : result.error)
    if (result.ok) refreshLiveData?.()
  }
  if (!live?.strategyRecommendations?.length) return <div className="wc-card"><SectionHead title="Clyde Strategy Cards" action="Analysis pending" /><p>Nexus is comparing bureau data and matching approved strategies. Low-confidence cases go to GoClear.</p></div>
  return <div className="wc-card"><SectionHead title="Clyde Strategy Cards" action={`${live.strategyRecommendations.length} automated option(s)`} /><p>Clyde separates Nexus-detected facts, client-provided facts, approved education, uncertainty, and next actions. Outcomes are not guaranteed.</p>{live.strategyRecommendations.map(rec => { const p=rec.payload||{}; const d=p.discrepancy||{}; const tools=p.availableTools||[]; const answer=answers[rec.id]||''; const strategyName=p.primaryStrategy||getCreditStrategyDisplayName(rec.strategy_id); return <div key={rec.id} className="wc-strategyCard" style={{border:'1px solid rgba(148,163,184,.25)',borderRadius:10,padding:12,marginTop:10}}><small>Approved educational strategy · version {rec.strategy_version} · evidence {rec.confidence||'moderate'}</small><h3>{strategyName}</h3><p><b>Nexus detected:</b> {d.differenceSummary||p.rationale}</p><small>{Object.entries(d.bureauValues||{}).map(([b,v])=>`${b}: ${v}`).join(' · ')}</small><p><b>Why it may apply:</b> {p.rationale||'The structured discrepancy matches this approved strategy.'}</p><p><b>Readiness relevance:</b> {d.fundingImpact||'May affect Credit Profile and Tier 1/Tier 2 readiness.'} Status: action needed.</p><p><b>Evidence that may help:</b> {(p.requiredEvidence||[]).join(', ')||'Relevant report pages or supporting records.'}</p><p><b>Limitations:</b> Nexus cannot guarantee correction, removal, score change, or funding approval.</p>{(p.clientConfirmationQuestions||[]).map(q=><p key={q}><b>Question only you can answer:</b> {q}</p>)}<label>What should Nexus know that the report cannot determine?<input aria-label="Client factual answer" value={answer} onChange={e=>setAnswers(x=>({...x,[rec.id]:e.target.value}))} /></label><div className="wc-actionRow"><button onClick={()=>choose(rec,'selected',undefined,answer)}>This matches my situation</button><button onClick={()=>choose(rec,'saved',undefined,answer)}>I need more information</button><button onClick={()=>choose(rec,'declined',undefined,answer)}>This does not apply</button><button onClick={async()=>{await choose(rec,'evidence_requested',undefined,answer);setEvidenceFor(rec.id)}}>I have supporting evidence</button><button onClick={()=>choose(rec,'saved',undefined,answer)}>I do not have supporting evidence</button><button onClick={()=>choose(rec,'saved',undefined,answer)}>Save and return later</button>{tools[0]&&<button onClick={()=>choose(rec,'draft_requested',tools[0],answer)}>Prepare Draft</button>}<button onClick={()=>navigate('/client/request-review?topic=credit-strategy-exception')}>Request GoClear Exception Review</button></div>{evidenceFor===rec.id&&<div><p>Upload evidence securely without leaving this workflow.</p><DocumentUploadZone compact maxFiles={1} category="dispute_support" sourceConcept="credit_strategy_evidence" fromPage="clyde_strategy_card" onUploadComplete={async details=>{if(details?.documentId&&selectionIds[rec.id])await linkStrategyEvidence({selectionId:selectionIds[rec.id],tenantId:live.resolvedTenantId,clientId:live.resolvedClientId,documentId:details.documentId,reportId:rec.document_id||p.documentId||'unknown',discrepancyId:rec.discrepancy_id||d.discrepancyId});setMessage('Evidence uploaded to Documents Vault and linked to this strategy when the saved selection was available.')}} /></div>}{decisions.get(rec.id)&&<small>Saved decision: {decisions.get(rec.id)}</small>}</div>})}{message&&<p>{message}</p>}</div>
}

function CreditPanel({ path, live, refreshLiveData, navigate, existingDocuments, onUploaded, openUploadPanel, readiness, journey }) {
  ...
  const accountRows = existingDocuments.filter(doc => /credit|report|statement|account|bureau/i.test(`${doc.category || ''} ${doc.title || ''} ${doc.filename || ''}`))
  if (path === '/client/account-details') return <section className="wc-panel wc-panel-credit"><SectionTabs tabs={creditTabs} currentPath={path} navigate={navigate} /><Hero variant="credit" eyebrow="Credit" title="Account Details" text="Review bureau reporting, account grouping, balances, discrepancies, documents, and GoClear review state without silently merging uncertain records." /><div className="wc-card wc-documentVault"><SectionHead title="Canonical Account Review" action={accountRows.length ? `${accountRows.length} record(s)` : 'Awaiting credit report'} />{accountRows.length ? accountRows.map((doc, i) => <div className="wc-vaultRow" key={doc.id || `${doc.title}-${i}`}><b>{doc.title || doc.filename || 'Credit account record'}</b><span>{doc.category || 'credit document'}</span><span>{clientDocumentProgress(doc)}</span><span>{doc.exception_review_status === 'required' ? 'Needs review' : 'Not merged automatically'}</span><span>{doc.created_at ? new Date(doc.created_at).toLocaleDateString() : 'Uploaded'}</span><span>Credit Journey</span></div>) : <div className="wc-emptyState"><div className="wc-softIcon">☁</div><h3>Awaiting account data</h3><p>Upload a current credit report before Nexus can show bureau-level account details, discrepancies, or canonical account grouping.</p><button onClick={openCreditUpload}>Upload Credit Report</button></div>}</div><CreditRepairCaseEnginePanel existingDocuments={existingDocuments} onUploaded={onUploaded} navigate={navigate} openUploadPanel={openUploadPanel} /></section>
  return <section className="wc-panel wc-panel-credit"><SectionTabs tabs={creditTabs} currentPath={path} navigate={navigate} /><Hero variant="credit" eyebrow="Credit Review" title={path === '/client/credit-utilization' ? 'Credit Utilization' : path === '/client/account-details' ? 'Account Details' : 'Credit Review'} text="Nexus analyzes your credit report and explains what may affect funding readiness. Outsourced dispute fulfillment (CRJ / DisputeForMe) performs dispute services — never from this screen." /><ClientGuidedSurface routeKey="credit" readiness={readiness} journey={journey} existingDocuments={existingDocuments} openUploadPanel={openUploadPanel} onUploaded={onUploaded} navigate={navigate} /><div className="wc-card wc-creditFlowIntro"><SectionHead title="Credit Review" action={hasReport ? 'Report analyzed - review pending' : 'Report needed'} /><p>Your Credit Review is your starting credit snapshot. Nexus reviews report information, utilization, inquiries, and possible documentation issues so you can understand what may be affecting your readiness.</p><p className="wc-modelNote"><b>Nexus analyzes. The fulfillment provider fulfills.</b> Nexus identifies what may be inaccurate, verifies before/after outcomes, and coordinates the outsourced fulfillment provider (CRJ / DisputeForMe) when a client authorizes dispute services. This page is analysis and education — no dispute is written, mailed, or submitted from here.</p>{hasReport && <p><b>Your report has been analyzed.</b> Nexus is reviewing the recommendations before any fulfillment next step is proposed.</p>}{approvedReview && <p><b>Approved readiness summary:</b> {String(approvedReview.summary?.creditProfileStatus || 'reviewed').replaceAll('_', ' ')} - {approvedReview.report_item_reviews?.length || 0} report item(s), {approvedReview.utilization_actions?.length || 0} utilization action(s), {approvedReview.evidence_needed?.length || 0} evidence request(s). <button onClick={() => navigate('/client/dispute-review')}>Open Safe Draft Review</button></p>}<div className="wc-trackPills">{['Credit report status', 'Report review status', 'Utilization status', 'Outsourced fulfillment'].map((label, i) => <span key={label}>{label}: {i === 0 ? hasReport ? 'In review' : 'Missing' : i === 3 ? 'Coordinated by Nexus' : 'In progress'}</span>)}</div></div><div className="wc-card wc-reportFirst"><SectionHead title="Start with Your Credit Report" action="Report-first flow" /><div className="wc-actionRow four">{entryOptions.map(option => <ActionCard key={option.id} icon={option.id === 'upload_credit_report' ? '☁' : option.id === 'connect_monitoring_resource' ? '🔗' : option.id === 'need_help_getting_report' ? '🎧' : '✎'} title={option.title} text={option.description} button={option.enabled ? 'Start' : 'View Options'} onClick={() => option.id === 'upload_credit_report' ? openCreditUpload() : navigate(option.route)} />)}</div></div><div className="wc-scoreFactors"><SectionHead title="Clyde Recommendations" action="Simple next steps" /><div className="wc-factorRow">
    {[
...
function BusinessPanel({ path, live, navigate, openUploadPanel }) {
  ...
  return <section className="wc-panel wc-panel-business"><SectionTabs tabs={businessTabs} currentPath={path} navigate={navigate} /><Hero variant="business" eyebrow="Business" title={path === '/client/business-credit' ? 'Business Credit' : path === '/client/business-bankability' ? 'Business Bankability' : path === '/client/business-setup' ? 'Business Setup' : 'Business Journey'} text="Build entity consistency, bankability, business credit, and documentation into a fundable business profile." /><div className="wc-card wc-checklist"><SectionHead title={path === '/client/business-credit' ? 'Business Credit Profile' : path === '/client/business-bankability' ? 'Bankability Requirements' : path === '/client/business-setup' ? 'Business Setup Checklist' : 'Business Journey'} action={`${checklist.length} setup items`} /><p>Complete your business foundation so GoClear can prepare you for Business Funding.</p><p>Tier 1 Business Profile: <b>{businessReadiness.tier1Status.replaceAll('_', ' ')}</b> - Tier 2 Business Profile: <b>{businessReadiness.tier2Status.replaceAll('_', ' ')}</b></p><div className="wc-checkGrid">{checklist.map(({ icon, title, status }) => <div className="wc-checkTile" key={title}><div className="wc-softIcon">{icon}</div><div><b>{title}</b><p>{path === '/client/business-bankability' ? 'Bankability signal' : path === '/client/business-credit' ? 'Business credit requirement' : 'Business foundation item'}</p><span>{String(status).replaceAll('_', ' ')}</span></div></div>)}</div><div className="wc-setupGrid">{[
...
function DisputeReviewPanel({ navigate, existingDocuments, onUploaded, openUploadPanel }) {
  ...
  return <section className="wc-panel wc-panel-dispute"><SectionTabs tabs={creditTabs} currentPath="/client/credit-repair-journey" navigate={navigate} /><Hero variant="creditRepair" eyebrow="Credit" title="Bureau Letter Builder" text="Review draft letters, evidence, edit requests, and approval gates before any mailing action." /><div className="wc-card wc-disputeHero"><div><h2>Draft Letter Review</h2><p>Review your draft letters, request edits, approve them, and authorize mailing when ready.</p></div><button onClick={() => navigate('/client/credit-repair-journey')}>Back to Credit Repair</button></div><div className="wc-card wc-safetyBanner"><b>DocuPost safety gate</b><p>DocuPost sending is enabled only after specialist review and client approval/e-sign authorization. Nothing is auto-sent from this page.</p></div><div className="wc-disputeGrid"><div className="wc-card wc-disputeList"><SectionHead title="Letter Review Queue" action={loading ? 'Loading...' : `${reviewable.length} letter(s)`} />{error && <p className="wc-errorText">{error}</p>}{notice && <p className="wc-successText">{notice}</p>}{!loading && !reviewable.length && <div className="wc-emptyState"><div className="wc-softIcon">✉</div><h3>No letters ready yet</h3><p>Upload a current credit report and complete specialist review before letters appear here.</p><button onClick={() => navigate('/client/credit-repair-journey')}>Continue Credit Repair</button></div>}{reviewable.map(letter => {
...
function ResourcesPanel({ live, navigate }) {
  ...
  return <section className="wc-panel wc-panel-resources"><Hero eyebrow="Resources" title="Funding Readiness Resources" text="Use approved educational resources for credit, business setup, bankability, documents, and review preparation." /><div className="wc-resourceRec">{cards.map(([tag, title, button]) => <div className="wc-card wc-resCard" key={title}><span>{tag}</span><h3>{title}</h3><p>Recommended based on your current goals.</p><button onClick={() => navigate(routeForResource(tag, title))}>{button}</button></div>)}</div><div className="wc-card wc-resourceBanner"><h3>Resources connect to real progress.</h3><p>Every article, video, guide, and tool helps you take action and move closer to funding.</p><div><span>✓ Learn proven strategies</span><span>✓ Take action with confidence</span><span>✓ Unlock more opportunities</span></div></div><div className="wc-catRow">{[['🎓', 'Learning Center', 18], ['📘', 'Funding Education', 22], ['📈', 'Credit Report Review Tools', 15], ['💼', 'Business Setup Guides', 17], ['🧰', 'Recommended Tools', 12]].map(([icon, title, count]) => <MiniCard key={title} icon={icon} title={title} text={`${count} resources available.`} button="Explore" onClick={() => navigate(routeForResource(title, title))} />)}</div><div className="wc-partnerRow">{(offers.length ? offers.slice(0, 4).map(o => [o.title || 'Recommended Tool', o.category || 'Recommended Tools']) : [['LiveWell', 'Credit Monitoring'], ['Nav', 'Business Credit Builder'], ['Relay', 'Business Banking'], ['D&B', 'Business Credit Profile']]).map(([name, text]) => <div className="wc-card wc-partner" key={name}><b>{name}</b><p>{text}</p><button onClick={() => navigate(routeForResource(text, name))}>Learn more →</button></div>)}</div></section>
}

function RecommendationsPanel({ live, readiness, existingDocuments, navigate, openUploadPanel }) {
  const storedRecommendations = live?.strategyRecommendations || []
  const activeTasks = (live?.tasks || []).filter(task => /recommend|strategy|document|review/i.test(`${task.category || ''} ${task.task_type || ''} ${task.title || ''}`))
  const docs = getLiveDocuments(live)
  const recommendations = storedRecommendations.length
    ? storedRecommendations.map((rec, index) => {
      const payload = rec.payload || {}
      const discrepancy = payload.discrepancy || {}
      return {
        id: rec.id || `${rec.strategy_id}-${index}`,
        priority: rec.priority || (index === 0 ? 'high' : 'medium'),
        category: payload.category || rec.category || 'Credit readiness',
        title: payload.primaryStrategy || rec.strategy_id || 'Review recommended strategy',
        reason: payload.rationale || discrepancy.differenceSummary || 'Nexus found a readiness item that needs client review before GoClear can act.',
        evidence: (payload.requiredEvidence || []).join(', ') || 'Current report details or supporting document',
        impact: discrepancy.fundingImpact || 'May affect funding readiness after verified review.',
        status: live?.strategyDecisions?.find?.(decision => decision.recommendation_id === rec.id)?.decision || 'recommended',
    ...
    : activeTasks.length
      ? activeTasks.slice(0, 6).map(task => ({
        ...
        title: task.title || 'Review recommended action',
        reason: task.summary || task.recommended_next_action || 'This item is waiting for client or GoClear review.',
        evidence: task.source_concept || 'Portal activity',
        impact: 'Keeps the review workflow moving without making unsupported outcome claims.',
        status: task.status || 'recommended',
        route: '/client/request-review',
      }))
      : []

  return <section className="wc-panel wc-panel-recommendations"><Hero eyebrow="Recommendations" title="Funding Readiness Recommendations" text="Review prioritized actions, evidence, status, and GoClear review requirements from the same Nexus 3.0 workflow system." /><div className="wc-recommendationSummary">
    <MiniCard icon="⚑" title="Readiness State" tag={readiness?.state ? readiness.state.replaceAll('_', ' ') : 'Incomplete'} text={readiness?.primaryBlocker || 'Upload documents and complete intake before Nexus can rank every action.'} button="Open readiness" onClick={() => navigate('/client/funding-readiness')} />
    <MiniCard icon="▤" title="Evidence Available" tag={`${existingDocuments.length} file(s)`} text={`${docs.missing.length} missing requirement(s) remain visible in the document workflow.`} button="Upload evidence" onClick={() => openUploadPanel({ track: 'recommendations', pageContext: 'recommendations', suggestedCategory: 'supporting_evidence', title: 'Upload Recommendation Evidence', description: 'Upload one supporting document for GoClear review.' })} />
    <MiniCard icon="👤" title="Review Gate" tag="Specialist gated" text="Clients can accept, upload evidence, or request review, but cannot approve administrative verification." button="Request review" onClick={() => navigate('/client/request-review?topic=recommendation-review')} />
  </div>{recommendations.length ? <div className="wc-recommendationGrid">{recommendations.map(item => <article className="wc-card wc-recommendationCard" key={item.id}><div className="wc-recTop"><span className={`wc-statusPill ${item.priority === 'high' ? 'orange' : ''}`}>{item.priority}</span><span>{item.category}</span></div><h3>{item.title}</h3><p><b>Reason:</b> {item.reason}</p><p><b>Supporting evidence:</b> {item.evidence}</p><p><b>Readiness impact:</b> {item.impact}</p><p><b>Status:</b> {String(item.status).replaceAll('_', ' ')}</p><div className="wc-actionRow"><button onClick={() => navigate(item.route)}>Start</button><button onClick={() => openUploadPanel({ track: 'recommendations', pageContext: 'recommendations', suggestedCategory: 'supporting_evidence', title: `Upload Evidence for ${item.title}`, description: 'Upload one document tied to this recommendation.' })}>Upload evidence</button><button onClick={() => navigate('/client/request-review?topic=recommendation-review')}>Submit for review</button></div></article>)}</div> : <div className="wc-card wc-emptyState"><div className="wc-softIcon">⚑</div><h3>No recommendations are ready yet</h3><p>Nexus will show recommendations after profile information, documents, and processing results are available. Nothing is fabricated when data is incomplete.</p><button onClick={() => openUploadPanel({ track: 'recommendations', pageContext: 'recommendations', suggestedCategory: 'credit_report', title: 'Upload a Starting Document', description: 'Upload one document so Nexus can begin readiness review.' })}>Upload a starting document</button></div>}<div className="wc-card wc-safetyBanner"><b>Customer-safe recommendation policy</b><p>Recommendations are readiness guidance only. Nexus does not guarantee funding approval, credit-score improvement, deletions, or lender outcomes.</p></div></section>
}

function ReviewPanel({ live, scores, readiness, journey, refreshLiveData, existingDocuments, openUploadPanel, navigate }) {
...
function CreditImprovementPanel({ live, navigate, existingDocuments, onUploaded, openUploadPanel, journey, readiness, scores }) {
  ...
  const progressSinceStart = Math.min(100, (scores?.repair || 0) + (verifiedCount * 8))
  return <section className="wc-panel wc-panel-improvement"><Hero eyebrow="Credit Improvement" title="Credit Improvement" text={OUTSOURCED_FULFILLMENT_EXPLANATION} /><div className="wc-card wc-fulfillmentHero"><div className="wc-fulfillmentLead"><div className="wc-softIcon">↻</div><div><span className={`wc-statusPill ${fulfillment.tone}`}>{fulfillment.statusLabel}</span><h3>Round {fulfillment.currentRound || '—'} · {fulfillment.expectedMilestone}</h3><p>{fulfillment.description}</p></div></div><div className="wc-fulfillmentMeta"><div><small>Current round</small><b>{fulfillment.currentRound || 0}</b></div><div><small>Verified outcomes</small><b>{fulfillment.verifiedOutcomes}</b></div><div><small>Progress since start</small><b>{progressSinceStart}%</b></div></div></div>
    <div className="wc-improvementGrid"><div className="wc-card wc-listCard"><SectionHead title="Next client action" action={fulfillment.statusId} /><div className="wc-actionCard" style={{ minHeight: 120 }}><div className="wc-softIcon">🎯</div><b>{fulfillment.nextClientAction}</b><button onClick={() => navigate(fulfillment.nextClientRoute)}>Continue →</button></div>{fulfillment.documentsNeeded.length > 0 && <div className="wc-inlineRequirementGrid">{fulfillment.documentsNeeded.map((label, i) => <InlineDocumentRequirement compact key={label} title={label} description={`Upload ${label.toLowerCase()} so Nexus can move the round forward.`} category="credit_report" requirementKey={i === 0 ? 'credit_report' : 'dispute_support'} fromPage="credit-improvement" impactLabel="Required" existingDocuments={existingDocuments} onUploaded={onUploaded} onOpenUpload={({ title, category, fromPage, description }) => openUploadPanel({ track: 'credit_repair', pageContext: fromPage || 'credit_improvement', suggestedCategory: category, title: `Upload ${title}`, description })} whyItMatters="Keeps the outsourced fulfillment round moving." />)}</div>}</div>
      <div className="wc-card wc-nextToFunding"><SectionHead title="Next step toward funding" action="Stage 3 of 5" /><Donut value={scores?.funding || 0} small tone="green" /><p>Credit improvement feeds directly into Business Foundation and Funding Readiness. Verified outcomes move your funding readiness score.</p><button onClick={() => navigate('/client/funding-readiness')}>View Funding Readiness →</button></div></div>
    <div className="wc-card wc-outcomes"><SectionHead title="Outcome Verification" action="Before / after comparison" /><p>Nexus compares your starting credit snapshot with bureau responses and labels every change. Only verified results are shown here.</p><div className="wc-outcomeGrid">{[['✓', 'Verified deletion', 'A negative item was confirmed removed by the bureau after the fulfillment round.', 'green'], ['↔', 'Non-deletion change', 'The item changed (status, balance, or details) but was not removed. Still a verified outcome.', 'blue'], ['!', 'Review required', 'The bureau response is ambiguous or needs documentation before it can be verified.', 'orange']].map(([mark, title, text, tone]) => <MiniCard key={title} icon={mark} title={title} tag="No results yet" text={text} />)}</div><p className="wc-safeNote">Friendly labels only — technical account-matching details are not shown here. Verification language never implies a guaranteed deletion.</p></div>
    <div className="wc-card wc-hermesHome"><div className="wc-bot"><img src={clydeHermesAgent} alt="Clyde advisor" /></div><h3>Clyde — current stage: Credit Improvement</h3><p>Ask Clyde what changed in your file, what the fulfillment provider is working on, or what we need from you next.</p><button onClick={() => navigate('/client/messages')}>Ask Clyde</button></div></section>
}

function FundingAccessPanel({ live, scores, navigate, existingDocuments, readiness, journey }) {
  const p = live?.profile || {}
  const business = evaluateBusinessFundingReadiness({ businessName: p.business_name, entityType: p.entity_type, stateRegistrationStatus: p.state_registration_status, einStatus: p.ein_status, businessAddress: p.business_address_line1, businessPhone: p.business_phone || p.phone, businessEmail: p.business_email, businessDomain: p.business_domain, naicsCode: p.naics_code, industry: p.industry, timeInBusiness: p.time_in_business, revenueRange: p.monthly_revenue_range, businessBankAccountStatus: p.business_bank_account_status, ownershipComplete: p.ownership_complete, licensesStatus: p.licenses_status, documents: getLiveDocuments(live).uploaded })
  const tiers = evaluateTierFundingReadiness({ credit: { status: scores.credit >= 80 ? 'ready_to_review' : scores.credit >= 65 ? 'almost_ready' : 'action_needed', dataSufficient: Boolean(existingDocuments.length) }, business: { status: business.status, completenessScore: business.completenessScore, timeInBusiness: !business.missingRequirements.includes('Time in business'), revenue: !business.missingRequirements.includes('Revenue range'), banking: !business.missingRequirements.includes('Business bank account status'), documentsComplete: business.documentRequests.length === 0 } })
  const ready = Boolean(readiness?.reviewEligible || journey?.reviewEligible) || scores.funding >= 75
  const resources = getClientResources({ placement: 'funding-access', limit: 3 })
  const applications = [
    { name: 'Starter business credit', status: ready ? 'Ready to apply' : 'Locked until readiness', route: '/client/request-review?topic=funding-review' },
    { name: 'Business credit cards', status: ready ? 'Ready to review' : 'Locked until readiness', route: '/client/request-review?topic=funding-options' },
    { name: 'Term loan / lender review', status: 'GoClear review required', route: '/client/request-review?topic=funding-review' },
  ]
  return <section className="wc-panel wc-panel-funding-access"><Hero eyebrow="Funding Access" title="Funding Access" text="Move from readiness to funding opportunities after GoClear confirmation. No live lender submission is connected — this page tracks your sequence and application state." /><div className="wc-card wc-accessGate"><SectionHead title="Readiness confirmation" action={ready ? 'Confirmed' : 'Not yet ready'} /><p className={ready ? 'wc-greenText' : 'wc-orangeText'}>{ready ? 'Your readiness snapshot supports moving to funding options. GoClear review still confirms the sequence.' : 'Complete Credit Review, Credit Improvement, Business Foundation, and the Funding Readiness requirements before applying. Funding is never promised.'}</p><div className="wc-trackPills"><span>Tier 1: {tiers.tier1.status.replaceAll('_', ' ')} — {tiers.tier1.score}</span><span>Tier 2: {tiers.tier2.status.replaceAll('_', ' ')} — {tiers.tier2.score}</span></div><button onClick={() => navigate(ready ? '/client/request-review?topic=funding-review' : '/client/funding-readiness')}>{ready ? 'Request GoClear funding review' : 'Complete funding readiness first'}</button></div>
    <div className="wc-accessGrid"><div className="wc-card wc-listCard"><SectionHead title="Recommended funding sequence" action="Educational" /><div className="wc-fundingSeq">{['Readiness confirmation', 'GoClear funding review', 'Application readiness', 'Lender submission (tracked)', 'Offers & approvals', 'Capital secured'].map((label, i) => <div key={label}><span className={i === 0 && ready ? 'done' : i === 0 ? 'active' : ''}>{i + 1}</span><b>{label}</b><p>{i < 2 ? (i === 0 ? (ready ? 'Confirmed' : 'Pending') : 'Specialist gated') : i === 4 ? 'Tracked here' : 'Tracked here'}</p></div>)}</div></div>
      <div className="wc-card wc-listCard"><SectionHead title="Application status" action="No live submissions" /><div className="wc-appTable">{applications.map(app => <div key={app.name} className="wc-appRow"><b>{app.name}</b><span className="wc-statusPill orange">{app.status}</span><button onClick={() => navigate(app.route)}>Ask GoClear</button></div>)}<p className="wc-safeNote">No lender application is submitted from this portal. Nexus and GoClear coordinate the sequence; lenders make final decisions.</p></div></div>
      <div className="wc-card wc-listCard"><SectionHead title="Offers received" action="None yet" /><div className="wc-emptyState"><div className="wc-softIcon">➤</div><h3>No offers tracked yet</h3><p>Approved amounts and activated amounts appear here only after a real, verified offer is received and GoClear confirms it.</p></div></div></div>
    <div className="wc-card wc-nextToFunding"><SectionHead title="Next step" action="Funding path" /><p>{ready ? 'Your next step is a GoClear funding review to confirm the recommended sequence and application readiness.' : 'Resolve the remaining readiness blockers first — Clyde can walk you through them.'}</p><button onClick={() => navigate(ready ? '/client/request-review?topic=funding-review' : '/client/messages')}>{ready ? 'Request funding review →' : 'Ask Clyde what is missing →'}</button>{resources.length > 0 && <div className="wc-resourceActions">{resources.map(resource => <button key={resource.title} onClick={() => navigate('/client/resources?category=funding-education')}>{resource.title}</button>)}</div>}</div></section>
}

function BillingPanel({ live, navigate }) {
  const verifiedCount = Number(live?.verifiedOutcomes || 0)
  const billedLines = [
    { id: 'round1-experian', label: 'Round 1 — verified deletion', round: 1, outlet: 'Experian' },
    { id: 'round1-equifax', label: 'Round 1 — verified deletion', round: 1, outlet: 'Equifax' },
  ]
  const lines = billingVisibility({ verifiedOutcomes: verifiedCount, verifiedDeletedIds: live?.verifiedDeletedIds || [], billedLines, paidIds: live?.paidBillingIds || [], approvedIds: live?.approvedBillingIds || [] })
  const stateTone = { pending_verification: 'orange', pending_review: 'blue', approved: 'blue', paid: 'green', reversed: 'orange' }
  return <section className="wc-panel wc-panel-billing"><Hero eyebrow="Billing" title="Billing & Payments" text="Readiness-review payments and proposed pay-per-delete charges. Nothing is billed unless outcome verification supports it." /><div className="wc-billingGrid"><div className="wc-card wc-listCard"><SectionHead title="Readiness Review payment" action="Test mode" /><div className="wc-appRow"><b>Credit & Funding Readiness Review</b><span className="wc-statusPill green">$0 — no paid service</span></div><p className="wc-safeNote">The readiness review is the only paid onboarding service. Verified payment records persist before any fulfillment work begins.</p></div>
      <div className="wc-card wc-listCard"><SectionHead title="Proposed pay-per-delete charges" action="Verification gated" /><div className="wc-billingRows">{lines.length === 0 && <p>No proposed charges yet.</p>}{lines.map(line => <div key={line.id} className="wc-billingRow"><div><b>{line.label}</b><small>Round {line.round} · {line.outlet}</small></div><div className="wc-billingAmt">{line.billable ? `$${line.proposedAmount}` : '—'}<span className={`wc-statusPill ${stateTone[line.state]}`}>{line.state.replaceAll('_', ' ')}</span></div></div>)}</div><p className="wc-safeNote">A deletion is never shown as billable unless outcome verification supports it. Approved charges are reviewed by GoClear before any payment is requested.</p></div>
      <div className="wc-card wc-listCard"><SectionHead title="Payment records" action="None yet" /><div className="wc-emptyState"><div className="wc-softIcon">◈</div><h3>No payment records yet</h3><p>Approved, paid, and reversed/credited charges will appear here with their verification evidence.</p></div></div></div>
    <div className="wc-card wc-caseSafety"><b>No automatic charging for credit deletion.</b><span>Nexus coordinates billing verification after verified outcomes. Payment authorization is tracked in your enrollment records, and every charge is itemized and reviewable.</span></div></section>
}

function MessagesPanel({ navigate, onOpenChat, openUploadPanel, clydeContext, contextMessages }) {
  const messages = contextMessages && contextMessages.length ? contextMessages : [
    { id: 'welcome', source: 'Clyde', text: 'Welcome to your guided journey. Ask me anything about your current stage.' },
    { id: 'upload', source: 'Clyde', text: 'Upload one document from any page — I will organize it for review.' },
  ]
  return <section className="wc-panel wc-panel-messages"><Hero eyebrow="Messages" title="Messages & Clyde" text="Ask Clyde about your current stage, what changed, what Nexus is working on, what the outsourced fulfillment provider is doing, or what we need from you next." /><div className="wc-msgGrid"><div className="wc-card wc-hermesHome"><div className="wc-bot"><img src={clydeHermesAgent} alt="Clyde advisor" /></div><h3>Clyde — your guide</h3><p>Clyde explains progress in plain language, supports missing-document requests, and points to your next step. Advisory only — not a decision.</p><div className="wc-clydeQuickActions"><button onClick={onOpenChat}>Ask Clyde now</button><button onClick={() => openUploadPanel({ track: clydeContext?.track || 'general', pageContext: 'messages', title: 'Upload One Document', description: 'Upload one document and Clyde will organize it for review.' })}>Upload a document</button></div></div>
      <div className="wc-card wc-msgList"><SectionHead title="Recent messages" action={`${messages.length} message(s)`} /><div className="wc-msgThread">{messages.slice(0, 8).map(message => <div key={message.id} className="wc-msgRow"><div className="wc-msgAvatar">✦</div><div><b>{message.source.replaceAll('_', ' ')}</b><p>{message.text}</p></div></div>)}</div><p className="wc-safeNote">Full conversation history with Clyde is available in the chat drawer on every page.</p></div></div></section>
}
...
function ClydeChatDrawer({ open, onClose, navigate, pageTitle, clydeContext, onClydeAction }) {
  ...
  return <div className="wc-clydeOverlay" role="dialog" aria-modal="true" aria-label="Ask Clyde">
    <div className="wc-clydeDrawer">
      <div className="wc-clydeDrawerHead"><div className="wc-bot"><img src={clydeHermesAgent} alt="Clyde advisor" /></div><div><h2>Ask Clyde</h2><p>{pageTitle} guidance</p></div><button onClick={onClose} aria-label="Close Clyde chat">×</button></div>
...
function ClydePanel({ navigate, onOpenChat, clydeContext, contextMessages = [], onClydeAction }) {
  ...
  return <aside className="wc-advisor"><div className="wc-advisorCard"><div className="wc-botTop"><div className="wc-bot"><img src={clydeHermesAgent} alt="Clyde advisor" /></div><div><h3>Clyde Funding Readiness Guide</h3><div className="wc-online">● Online</div></div></div><p>{next.pageGoal} I’ll show what is missing, what GoClear is reviewing, and what needs your approval.</p><div className="wc-advisorBox"><h4>Persisted context</h4>{contextMessages.slice(0, 3).map(message => <p className="wc-clydeContextLine" key={message.id}><b>{message.source.replaceAll('_', ' ')}</b>: {message.text}</p>)}</div><div className="wc-advisorBox"><h4>Top Recommendations</h4>{recommendations.map((item, i) => <button className="wc-clydeItem" key={item.title} onClick={() => onClydeAction(item)}><ListItem tone={item.priority === 'high' ? 'orange' : item.priority === 'medium' ? 'blue' : 'green'} mark={String(i + 1)} title={item.title} text={item.reason} /></button>)}</div><div className="wc-advisorBox"><h4>Clyde Tip</h4><p>Upload one document and I’ll organize it for GoClear review. Documents is your vault, but you can upload from any workflow page.</p></div><div className="wc-suggestions"><button onClick={() => onClydeAction({ actionType: 'upload', label: 'Upload Document', uploadContext: { track: next.currentTrack } })}>Upload Document</button><button onClick={() => navigate('/client/funding-readiness')}>What does funding need?</button></div><button className="wc-chatBtn" onClick={onOpenChat}>Chat with Clyde</button></div></aside>
}
...
export default function WorldClassClientPortal({ path, onNavigate }) {
  ...
  const activeNavKey = ['repair', 'dispute'].includes(meta.key) ? 'credit' : meta.key
  ...
  const clydeContext = useMemo(() => ({ route: path, track: meta.key === 'credit' ? 'credit_profile' : meta.key === 'improvement' || meta.key === 'repair' || meta.key === 'dispute' ? 'credit_repair' : meta.key === 'profile' || meta.key === 'business' ? 'business_profile' : meta.key === 'funding' ? 'business_funding' : meta.key === 'funding-access' ? 'business_funding' : meta.key === 'review' ? 'request_review' : meta.key === 'documents' ? 'documents' : meta.key === 'billing' ? 'billing' : meta.key === 'messages' ? 'messages' : 'general', clientState: { documents: existingDocuments, missingRequirements: getLiveDocuments(live).missing, profileComplete: profileComplete?.complete, pendingReview: live?.tasks?.some?.(t => t.status === 'pending_admin_review') } }), [path, meta.key, existingDocuments, live, profileComplete])
  ...
  const panel = showIcons ? <IconSystemPanel /> : {
    home: <HomePanel scores={scores} live={live} profileComplete={profileComplete} navigate={routeTo} openUploadPanel={openUploadPanel} readiness={readiness} journey={journey} existingDocuments={existingDocuments} onUploaded={refreshLiveData} />,
    ...
    credit: <CreditPanel path={path} live={live} refreshLiveData={refreshLiveData} navigate={routeTo} existingDocuments={existingDocuments} onUploaded={refreshLiveData} openUploadPanel={openUploadPanel} readiness={readiness} journey={journey} />,
    documents: <DocumentsPanel live={live} refreshLiveData={refreshLiveData} navigate={routeTo} openUploadPanel={openUploadPanel} readiness={readiness} journey={journey} existingDocuments={existingDocuments} />,
    business: <BusinessPanel path={path} live={live} navigate={routeTo} openUploadPanel={openUploadPanel} />,
    funding: <FundingPanel scores={scores} live={live} navigate={routeTo} existingDocuments={existingDocuments} onUploaded={refreshLiveData} openUploadPanel={openUploadPanel} readiness={readiness} journey={journey} />,
    improvement: <CreditImprovementPanel live={live} navigate={routeTo} existingDocuments={existingDocuments} onUploaded={refreshLiveData} openUploadPanel={openUploadPanel} journey={journey} readiness={readiness} scores={scores} />,
    'funding-access': <FundingAccessPanel live={live} scores={scores} navigate={routeTo} existingDocuments={existingDocuments} readiness={readiness} journey={journey} />,
    billing: <BillingPanel live={live} navigate={routeTo} />,
    messages: <MessagesPanel navigate={routeTo} onOpenChat={() => setClydeOpen(true)} openUploadPanel={openUploadPanel} clydeContext={clydeContext} contextMessages={contextClydeMessages} />,
    ...
    recommendations: <RecommendationsPanel live={live} readiness={readiness} existingDocuments={existingDocuments} navigate={routeTo} openUploadPanel={openUploadPanel} />,
    resources: <ResourcesPanel live={live} navigate={routeTo} />,
    review: <ReviewPanel live={live} scores={scores} readiness={readiness} journey={journey} refreshLiveData={refreshLiveData} existingDocuments={existingDocuments} openUploadPanel={openUploadPanel} navigate={routeTo} />,
  }[meta.key]
