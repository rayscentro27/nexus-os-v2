...
export function resolveRouteAdapter(path: string, data: V2ViewData): RouteAdapterState {
  const states: Record<string, RouteAdapterState> = {
    ...
    '/client-v2/bankability': { adapter: 'readiness + documents', backend: 'Supabase bankability/document data', hasData: data.readiness.stages.tier1.requirements.length > 0, permittedActions: ['view', 'upload supporting document'] },
