...
export function classifyTraceQuestion(message: string): TraceQuestionClassification | null {
  ...
  if (/\bwhat did you get (?:that|this|the) (?:last )?(?:answer|response) from\b|\bwhat part of your decision[- ]?(?:making )?process did you use\b|\bhow did you decide (?:that|this)\b|\bwhat made you (?:give|say|answer) (?:that|this|the) (?:response|answer)?\b|\bwhy did you (?:answer|give|say) (?:that|this|it|that way)\b|\bwhat (?:was|is) your (?:reasoning|rationale|basis|thinking) (?:for|behind|in) (?:that|this|the)\b|\bwhy (?:do|did) you (?:recommend|choose|pick|select|go with) (?:that|this|the|it)\b|\bwhat (?:made|caused) you (?:to|for) (?:choose|pick|select|recommend|go with)\b|\bwhat part of your decision process did you use\b/.test(lower)) return { kind: /decision[- ]?making|how did you decide|reasoning|rationale|basis|thinking/.test(lower) ? 'why' : 'source', target: 'last_answer' };
