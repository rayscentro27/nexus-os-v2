...
export interface WorkerJobInput {
  ...
  output_destination: string; requested_by: string
}
...
export function buildRestaurantCampaignProof(now = new Date().toISOString()): CampaignContractProof {
  ...
  return { transformation, marketing_input: marketingInput, marketing_brief: marketingBrief, creative_input: creativeInput, creative_brief: creativeBrief, creative_package: creativePackage, qa, approval, outcome_feedback, future_worker_input, future_worker_output,
