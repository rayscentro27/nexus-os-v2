export type ConversationIntent = 'favorite_food' | 'favorite_sport' | 'joke' | 'wellbeing' | 'opinion' | 'simple_explanation' | 'identity' | 'casual_followup';
...
export function classifyConversationIntent(message: string): ConversationIntent | null {
  ...
  if (/are you real|who are you|what are you/.test(text)) return 'identity';
...
export function answerConversation(message: string, intent = classifyConversationIntent(message)): string | null {
  switch (intent) {
    ...
    case 'identity': return "I'm Hermes, an AI advisor inside Nexus. I'm not human and I don't have a live model unless one is explicitly configured, but I can reason over approved context, live RLS-gated Supabase data, and operations reports when those sources are available.";
