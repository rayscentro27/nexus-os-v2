...
import type { NexusDepartment, NexusProject, NexusProjectStatus } from '../config/nexusProjectTypes';
...
function base(row: Row, patch: Partial<NexusProject>): NexusProject {
  ...
  return {
    ...
    status: patch.status ?? statusFrom(row.status ?? row.decision ?? row.review_status),
    score: patch.score ?? score(row.score ?? row.overall_score),
    priority: patch.priority ?? 'medium',
    summary: text(patch.summary, text(row.summary ?? row.source_summary ?? row.description ?? row.snippet, '')),
    pros: patch.pros ?? arr(row.pros ?? row.metadata?.pros),
    cons: patch.cons ?? arr(row.cons ?? row.metadata?.cons),
    recommendation: text(patch.recommendation, text(row.recommendation ?? row.recommended_action ?? row.smallest_test ?? row.diagnosis, 'Review the saved metadata and choose the next safe action.')),
    proposed_changes: patch.proposed_changes ?? arr(row.proposed_changes ?? row.metadata?.proposed_changes ?? row.revision_notes),
    proposed_schedule: text(patch.proposed_schedule, text(row.proposed_schedule ?? row.metadata?.proposed_schedule, getProjectScheduleLabel({ status: statusFrom(row.status ?? row.decision) } as NexusProject))),
    next_action: text(patch.next_action, text(row.next_action ?? row.recommended_fix ?? row.smallest_test ?? row.recommended_action, 'Review with Hermes.')),
    approval_required: patch.approval_required ?? Boolean(row.approval_required ?? row.payload?.approval_required ?? row.metadata?.approval_required),
    feedback_requested: patch.feedback_requested ?? /review|revise|pending/i.test(text(row.status ?? row.decision)),
    risk_triggers: patch.risk_triggers ?? arr(row.risk_triggers ?? row.payload?.risk_flags ?? row.metadata?.risk_triggers ?? row.claim_flags),
    ...
    proof_event_id: patch.proof_event_id ?? (text(row.proof_event_id ?? row.event_id, '') || null),
...
export function mapResearchSourceToProject(row: Row): NexusProject {
  ...
  return base(row, {
    ...
    status: score(m.total_opportunity_score) != null ? 'scored' : statusFrom(reviewStatus),
    score: score(m.total_opportunity_score ?? row.score),
    summary: text(row.snippet, ''),
    pros: arr(m.pros),
    cons: arr(m.cons),
    recommendation: text(m.recommended_action ?? m.recommended_destination, 'Saved. Summary/enrichment pending.'),
    proposed_schedule: text(m.proposed_schedule, 'Saved immediately; enrichment can run later.'),
    next_action: text(m.next_action, 'Review metadata or request safe enrichment.'),
...
export function mapTaskRequestToProject(row: Row): NexusProject {
  return base(row, {
    ...
    summary: text(row.summary ?? row.result_summary, ''),
    recommendation: text(row.payload?.recommendation, 'Review this safe request before assigning worker time.'),
    approval_required: Boolean(row.payload?.approval_required),
    risk_triggers: arr(row.payload?.risk_triggers ?? (row.payload?.review_trigger ? [row.payload.review_trigger] : [])),
...
export async function loadDepartmentProjects(tabId: string): Promise<NexusProject[]> {
  if (tabId === 'intake') {
    ...
    return [...sources.map(mapResearchSourceToProject), ...tasks.map(mapTaskRequestToProject).filter((p) => p.project_type.includes('source'))]
