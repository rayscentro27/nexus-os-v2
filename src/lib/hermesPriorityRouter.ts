...
export function routeHermesPriority(input: PriorityRouterInput): RouteDecision {
  ...
  if (promptKind === 'system_health') { const diagnostic = /where is the problem|what is the issue|what is not working/i.test(lower); return decision({ routeId: 'system_health_report', activationLevel: 1, domain: diagnostic ? 'system_diagnostics' : 'system_health', intent: diagnostic ? 'identify_problem' : 'status_contract', memoryPolicy: 'none', retrievalPolicy: 'local_reports', modelPolicy: 'forbidden', diagnosticsPolicy: 'hidden', actionPolicy: 'none', reason: 'A direct system-health question requires the system health response contract.' }); }
  ...
  if (/\b(where is the problem|what is the issue|what is not working|what is broken|system health report|report on the system health|what is system health|show system health)\b/i.test(lower)) return decision({ routeId: 'system_health_report', activationLevel: 1, domain: /where is the problem|what is the issue|what is not working/.test(lower) ? 'system_diagnostics' : 'system_health', intent: /where is the problem|what is the issue|what is not working/.test(lower) ? 'identify_problem' : 'report_request', memoryPolicy: 'none', retrievalPolicy: 'local_reports', modelPolicy: 'forbidden', diagnosticsPolicy: 'hidden', actionPolicy: 'none', reason: 'System-health and problem-identification questions require a user-facing local evidence summary.' });

  if (/\b(?:are you connected to (?:this|the) (?:website|page)|can you see (?:this|the) page|what about this page|what page am i on|what section is this|can you use the current page|are you reading this dashboard)\b/i.test(lower)) return decision({ routeId: /connected to/.test(lower) ? 'page_connection_status' : 'page_context_status', activationLevel: 1, domain: 'current_page', intent: /connected to/.test(lower) ? 'website_connection_status' : 'current_page_question', memoryPolicy: 'none', retrievalPolicy: 'none', modelPolicy: 'forbidden', diagnosticsPolicy: 'hidden', actionPolicy: 'none', reason: 'Current-page questions use only metadata passed by the chat surface and must remain separate from Supabase capability.' });
