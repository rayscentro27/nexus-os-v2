...
export function routeHermesPriority(input: PriorityRouterInput): RouteDecision {
  ...
  const promptKind = detectPromptKind(message);
  ...
  if (scopeKey && isSessionActive(scopeKey)) {
    ...
    const isTraceQuestion = /\b(?:where did|what source|what part of|how did you decide|why did you answer|was that (?:live|local)|what made you|why did you (?:answer|give|say)|what (?:was|is) your (?:reasoning|rationale|basis|thinking)|why (?:do|did) you (?:recommend|choose|pick|select)|what (?:made|caused) you)\b/i.test(lower);
  ...
  if (promptKind === 'access_map') return decision({ routeId: 'access_map', activationLevel: 1, domain: 'hermes_access', intent: 'capability_access_map', memoryPolicy: 'none', retrievalPolicy: 'local_reports', modelPolicy: 'forbidden', diagnosticsPolicy: 'hidden', actionPolicy: 'none', reason: 'Hermes access questions use the read-only capability registry and do not execute actions.' });
  ...
  if (promptKind === 'system_health') { const diagnostic = /where is the problem|what is the issue|what is not working/i.test(lower); return decision({ routeId: 'system_health_report', activationLevel: 1, domain: diagnostic ? 'system_diagnostics' : 'system_health', intent: diagnostic ? 'identify_problem' : 'report_request', memoryPolicy: 'none', retrievalPolicy: 'local_reports', modelPolicy: 'forbidden', diagnosticsPolicy: 'hidden', actionPolicy: 'none', reason: 'A direct system-health question requires the system health response contract.' }); }
  if (promptKind === 'approvals_pending') return decision({ routeId: 'explicit_domain_retrieval', activationLevel: 2, domain: 'approvals', intent: 'pending_status', memoryPolicy: 'none', retrievalPolicy: 'supabase_then_static_fallback', modelPolicy: 'forbidden', diagnosticsPolicy: 'hidden', actionPolicy: 'none', reason: 'A direct pending-approval question requires an authenticated read attempt before any clarification.' });
  if (promptKind === 'research_engine_status') return decision({ routeId: 'research_engine_status', activationLevel: 1, domain: 'research_youtube', intent: 'engine_status', memoryPolicy: 'none', retrievalPolicy: 'local_reports', modelPolicy: 'forbidden', diagnosticsPolicy: 'hidden', actionPolicy: 'none', reason: 'Research engine status has a dedicated report-backed response contract.' });
  if (promptKind === 'client_records') return decision({ routeId: 'client_records', activationLevel: 2, domain: 'clients', intent: 'record_inventory', memoryPolicy: 'none', retrievalPolicy: 'supabase_first', modelPolicy: 'forbidden', diagnosticsPolicy: 'hidden', actionPolicy: 'none', reason: 'A client inventory question requires a client_profiles read attempt and an exact verification blocker.' });

  if (promptKind === 'specialist_handoff') return decision({ routeId: 'specialist_handoff', activationLevel: 6, domain: domain === 'unknown' ? input.selectionMemory.activeDomain || 'delegation' : domain, intent: 'prepare_specialist_handoff', memoryPolicy: 'selection_only', retrievalPolicy: 'none', modelPolicy: 'forbidden', diagnosticsPolicy: 'hidden', actionPolicy: 'approval_required', reason: 'Specialist handoff is a dedicated conversation-only draft contract; nothing is assigned or sent.' });
  if (promptKind === 'specialist_agent_inventory') return decision({ routeId: 'specialist_agent_inventory', activationLevel: 2, domain: 'specialist_agents', intent: 'inventory_check', memoryPolicy: 'none', retrievalPolicy: 'local_reports', modelPolicy: 'forbidden', diagnosticsPolicy: 'hidden', actionPolicy: 'none', reason: 'Specialist agent inventory questions check the documented module/agent registry without model calls or selection memory.' });
  if (promptKind === 'external_current_info') return decision({ routeId: 'external_current_info', activationLevel: 1, domain: 'external_info', intent: 'current_info_request', memoryPolicy: 'none', retrievalPolicy: 'none', modelPolicy: 'forbidden', diagnosticsPolicy: 'hidden', actionPolicy: 'none', reason: 'External/current information questions are answered with honest capability limits; no live external lookup is available from this chat.' });
  ...
  if (/\b(where is the problem|what is the issue|what is not working|what is broken|what's broken|system health report|report on the system health|what is system health|what is our system health|how is the system health|show system health|nexus health|how healthy is nexus|is nexus healthy|what is working|what's working)\b/i.test(lower)) return decision({ routeId: 'system_health_report', activationLevel: 1, domain: /where is the problem|what is the issue|what is not working|what's broken/.test(lower) ? 'system_diagnostics' : 'system_health', intent: /where is the problem|what is the issue|what is not working|what's broken/.test(lower) ? 'identify_problem' : 'report_request', memoryPolicy: 'none', retrievalPolicy: 'local_reports', modelPolicy: 'forbidden', diagnosticsPolicy: 'hidden', actionPolicy: 'none', reason: 'System-health and problem-identification questions require a user-facing local evidence summary.' });

  if (/\b(?:are you connected to (?:this|the) (?:website|page|app|dashboard|section)|can you see (?:this|the) page|can you see what is on (?:this|the) page|what (?:color|colour) is (?:this|the|the nexus admin) page|what about this page|what page am i on|what page are we on|what page (?:is this|are we on|am i viewing)|what section is this|can you use the current page|are you reading this dashboard|what (?:section|part|area) of (?:the )?(?:app|website|page) (?:is this|are we on))\b/i.test(lower)) return decision({ routeId: /connected to/.test(lower) ? 'page_connection_status' : 'page_context_status', activationLevel: 1, domain: 'current_page', intent: /connected to/.test(lower) ? 'website_connection_status' : 'current_page_question', memoryPolicy: 'none', retrievalPolicy: 'none', modelPolicy: 'forbidden', diagnosticsPolicy: 'hidden', actionPolicy: 'none', reason: 'Current-page questions use only metadata passed by the chat surface and must remain separate from Supabase capability.' });
  ...
  if (isCasualCommonQuestion(message)) return decision({ routeId: 'casual_common', activationLevel: 1, domain: 'general_conversation', intent: /\bgood night\b|^night\b/.test(lower) ? 'farewell_or_light_check_in' : isHumanExperienceQuestion(message) ? 'human_experience_question' : /\b(favou?rite|joke|sky|pizza|movie|music|ice cream)\b/i.test(lower) ? 'casual_or_common_question' : 'greeting_or_light_check_in', memoryPolicy: 'none', retrievalPolicy: 'none', modelPolicy: 'forbidden', diagnosticsPolicy: 'hidden', actionPolicy: 'none', reason: 'Common conversation needs no Nexus records, selection memory, or model call.' });

  if (promptKind === 'advisory_followup' && advisory) return decision({ routeId: 'advisory_followup', activationLevel: 4, domain: advisory.lastAdvisoryDomain, intent: 'evaluate_prior_advice', memoryPolicy: 'long_term_allowed', retrievalPolicy: 'none', modelPolicy: 'allowed_if_needed', diagnosticsPolicy: 'hidden', actionPolicy: 'none', reason: 'A vague plan-level follow-up uses only the unexpired advisory lane after safety, provenance, status, action, and casual checks.' });
