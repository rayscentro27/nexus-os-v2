...
export type HermesPromptKind =
  ...
  | 'ray_review' | 'scheduling' | 'selection_reference' | 'casual_common'
...
export function isSystemHealthQuestion(message: string): boolean {
  return /\b(?:how|what) (?:is|does) (?:the )?system health|\bsystem health\b.*\b(?:status|working|look|report|check)|\b(?:what is broken|where is the problem|what is not working)\b/i.test(message);
}
