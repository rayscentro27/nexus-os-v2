...
PERSONAS={'a':'nexus-persona-a-browser@goclear.test'}
FIXTURE=ROOT/'test_fixtures/credit_reports/temp_client_alex_morgan_3_bureau_tradeline_report.pdf'
def envfile(path):
 d={}
 if path.exists():
...
def main():
 ...
 if not url or not key or not FIXTURE.exists(): print('FAIL: required server environment or synthetic fixture unavailable');return 1
 ...
 tenant,client=membership[0]['tenant_id'],membership[0]['client_id'];title='synthetic_persona_a_three_bureau_report.pdf'
