...
def build(args):
 ...
 existing=read_json(SUPABASE_READY/"youtube_video_metadata_latest.json",[]);merged={x.get("id"):x for x in existing+metadata};write_json(SUPABASE_READY/"youtube_video_metadata_latest.json",list(merged.values()));write_json(SUPABASE_READY/"youtube_ytdlp_probe_status_latest.json",metadata);write_json(SUPABASE_READY/"youtube_subtitle_availability_latest.json",subtitles)
