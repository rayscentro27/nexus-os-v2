...
def build():
 ...
 cards=[record(f"notebooklm-approval-{i+1}","approval_card",f"Approve NotebookLM source: {x['title']}",source_item_id=x["id"],status="pending_Ray_review",risk_level="medium",automation_level="approval_required",approval_required=True) for i,x in enumerate(sources)]
