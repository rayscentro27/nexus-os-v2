...
def main() -> int:
    ...
    follow_up = {"status": "NOT_NEEDED", "question": None, "source": None}
    alpha_after = None
    # Alpha's unknowns trigger one bounded Research-owned follow-up.  Use the
    # next real URL already present in the existing Research record; Ray does
    # not supply support URLs.
    follow_candidates = sources[3:] or [{"url": "https://azdot.gov/sites/default/files/media/2020/01/Vehicle-Wash-Manual.pdf", "source": "ADOT Vehicle Wash Manual", "kind": "regulatory_constraint"}]
    if alpha.get("research_requests") and follow_candidates[0].get("url"):
        follow_url = follow_candidates[0]["url"]
        try:
            follow_raw, follow_headers = fetch(follow_url)
            follow_source = {"source_id": "opportunity-follow-up-1", "source_type": "WEB_PAGE", "source_url": follow_url, "source_title": follow_candidates[0].get("source", follow_url), "source_class": follow_candidates[0].get("kind", "regulatory"), "acquired": True}
            follow_claims = extract_claims(follow_raw[:12000], follow_source)[:3]
            for claim in follow_claims: claim["verification_status"] = "PARTIALLY_SUPPORTED"
            package["evidence"].append({"source": follow_source, "question_answered": alpha["research_requests"][0]["question"], "provenance": {"source_url": follow_url, "excerpt": follow_raw[:500]}})
            package["claims"].extend(follow_claims)
            package["unknowns"] = package["unknowns"][1:]
            alpha_after = alpha_collaborative_analysis(package, proposed_action="PRELIMINARY_BUSINESS_PLAN")
            follow_up = {"status": "COMPLETED", "question": alpha["research_requests"][0]["question"], "source": follow_source, "claims_created": len(follow_claims)}
            source_rows.append(follow_source); acquired.append(follow_url)
        except Exception as exc:
            follow_up = {"status": "FAILED_RETRYABLE", "question": alpha["research_requests"][0]["question"], "source": follow_url, "error": str(exc)}
    ...
    records = {"sources": source_rows + [idea], "claims": claims, "opportunities": [{"opportunity_id": thesis["opportunity_thesis_id"], "title": thesis["title"], "goclear_fit": "LOW", "new_venture_potential": "ELIGIBLE", "research_continues": True, "status": "THESIS_ONLY_NO_EXECUTION"}], "questions": questions, "investigations": [{"investigation_id": package["research_package_id"], "package": package, "decision": investigation_decision(kind="OPPORTUNITY", researchability=.8, testability=.7, upside=.6, decision_relevance=.7)}], "alpha_reviews": [{"research_package_id": package["research_package_id"], **alpha}, *([{"research_package_id": package["research_package_id"], "review_stage": "AFTER_FOLLOW_UP", **alpha_after}] if alpha_after else [])], "plans": [plan], "reputations": [reputation], "handoffs": [handoff], "follow_ups": [{"parent_package_id": package["research_package_id"], "questions": package["unknowns"], "status": follow_up["status"], "bounded": True, "follow_up": follow_up}]}
    counts = persist_v2_records(records)
    knowledge = knowledge_maturity(0)
    report = {"status": "PASS_REAL", "initial_source": idea, "ray_supplied_support_urls": False, "questions_generated": questions, "searches_performed": [x["query"] for x in questions], "sources_discovered": len(sources), "sources_acquired": len(acquired), "source_classes": sorted({x["source_class"] for x in source_rows}), "package": package, "alpha_review": alpha, "alpha_follow_up_review": alpha_after, "follow_up": follow_up, "plan": plan, "handoff": handoff, "reputation": reputation, "knowledge_canary": {"source_processed": "existing GitHub Research artifact", "knowledge_captured": True, **knowledge, "opportunity_created": False}, "durable_record_counts": counts, "metrics": productivity_metrics([{"acquired": True, "processing_status": "FULLY_PROCESSED", "substantive_findings": claims, "summary_created": True, "extraction_created": True, "follow_up_questions": questions}]), "safety": {"opportunities_executed": 0, "work_orders_created": 0, "external_action": False}}
