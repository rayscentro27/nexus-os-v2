...
def build():
    ...
    ready=cli["test_mode_verified"] or (cli["installed"] and env["test_secret_detected"] and not env["live_secret_detected"])
    report={"ok":True,"generated_at":now(),"status":"stripe_test_mode_ready_for_Ray_approval" if ready else "stripe_test_setup_incomplete",
            ...
            "legacy_payment_files":legacy,"raw_values_included":False,"stripe_read_only_test_request_performed":cli["read_only_test_request_performed"],"stripe_api_write_performed":False,"real_charge_created":False,"external_action_performed":False}
