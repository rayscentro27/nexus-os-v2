...
ENDPOINT="http://127.0.0.1:8787/api/stripe/webhook"
def build():
 source_matches=[]
 ...
 port_open=False
 try:
  with socket.create_connection(("127.0.0.1",8787),timeout=2):port_open=True
 except OSError:pass
 ...
 endpoint_exists=(ROOT/"scripts/payments/stripe_webhook_test_server.py").exists()
 listener="stripe listen --forward-to localhost:8787/api/stripe/webhook"
 triggers=["stripe trigger checkout.session.completed","stripe trigger payment_intent.succeeded"]
 public={"id":"stripe-webhook-test-status","tenant_id":"tenant_test_goclear","client_id":"client_test_julius_erving","category":"stripe_webhook_test_status","title":"Stripe local webhook test","status":"endpoint_missing_implementation_required" if not endpoint_exists else "endpoint_ready_for_test_listener","test_mode":True,"endpoint":ENDPOINT,"port_8787_open":port_open,"endpoint_http_status":status,"listener_started":False,"trigger_attempted":False,"event_received":False,"approval_required":True,"external_action_performed":False,"created_at":now()}
 write_json(SUPABASE_READY/"stripe_webhook_test_status_latest.json",[public])
 report={"ok":True,"generated_at":now(),"status":public["status"],"local_endpoint_exists":endpoint_exists,"source_matches":source_matches,"port_8787_open":port_open,"endpoint_http_status":status,"endpoint_content_type":content_type.split(";",1)[0],"listener_ready":endpoint_exists,"listener_started":False,"trigger_attempted":False,"event_received":False,"server_command":"python3 scripts/payments/stripe_webhook_test_server.py","listener_command":listener,"test_trigger_commands":triggers,"webhook_secret_printed":False,"real_charge_created":False,"external_action_performed":False,"next_required_action":"Implement and approve a local signature-verifying webhook endpoint before starting the listener or triggers." if not endpoint_exists else "Approve starting the test server, Stripe listener, and synthetic triggers."}
