...
from nexus_agent_platform.loops.business import SELECTED_BUSINESS_LOOPS, eligibility_matrix, run_business_loop
...
def main() -> Dict[str, Any]:
    ...
    for spec in SELECTED_BUSINESS_LOOPS:
        ...
        first = runtime.run(spec, {"records": proof_records, "mode": "bounded_internal_phase14", "proof_id": "phase14_initial_loop_proof_v7"})
        second = runtime.run(spec, {"records": proof_records, "mode": "bounded_internal_phase14", "proof_id": "phase14_initial_loop_proof_v7"})
    ...
    report = {"phase": "PHASE 14 — CONTROLLED BUSINESS LOOP EXPANSION", "mode": "bounded_internal_non_publishing", "generated_at": datetime.now(timezone.utc).isoformat(), "selected_loops": [spec.loop_id for spec in SELECTED_BUSINESS_LOOPS], "eligibility": eligibility_matrix(), "contracts": [_contract(spec) for spec in SELECTED_BUSINESS_LOOPS], "runs": runs, "governance": {"client_portal_changes": "NONE", "production_telegram_changes": "NONE", "public_publishing": "DISABLED", "mass_outreach": "DISABLED", "paid_actions": "DISABLED", "provider_credits": "DISABLED", "production_deployment": "DISABLED", "client_pii": "NOT_USED", "phase12_promotion": "NOT_PERFORMED"}, "learning_telemetry": {"emitted": ["no_change_runs", "ai_calls", "tokens", "provider_cost", "success_failure", "retries", "value_events", "duplicate_work_avoided"], "automatic_policy_mutation": False}}
