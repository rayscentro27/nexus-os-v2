...
def refresh_mission_control_snapshot(*, scheduler_health_path: Path) -> Dict[str, Any]:
    ...
    snapshot = {
        ...
        "executive_portfolio": {
            ...
            "planned": portfolio.get("plan", {}).get("selected", []),
            "execution": portfolio.get("plan", {}).get("execution", {}),
            "dispatched": portfolio.get("plan", {}).get("execution", {}).get("dispatched", []),
            "materially_advanced": portfolio.get("plan", {}).get("execution", {}).get("materially_advanced", []),
