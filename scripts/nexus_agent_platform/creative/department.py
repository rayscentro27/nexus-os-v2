...
def _screenshot(url: str, path: Path, viewport: str) -> None:
    ...
    # The Python 3.14 Playwright driver can remain alive after file:// capture
    # on this host. The Node API exits cleanly with explicit context/browser
    # teardown, while preserving the same real Chromium rendering contract.
    runner = Path(__file__).resolve().parents[2] / "creative" / "playwright_screenshot.mjs"
    subprocess.run(["node", str(runner), url, str(path), str(width), str(height)], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=45)
