...
def classify(message):
 m=message.lower()
 for intent,pattern in [('greeting',r'good morning|hello|^hi\b|^hey\b'),('coffee',r'coffee'),('casual',r'did you sleep|how are you|sleep well'),('approval',r'what needs my approval|what.*approv'),('empathy',r'frustrated|fake ui|ui was fake'),('partner',r'talk to me like a partner|command bot|talk.*partner'),('today_plan',r'plan for today|today.*plan'),('next_steps',r'what should i do next|next move|priority'),('delegation',r'100 steps|delegate|turn this into'),('money',r'money|revenue|offer'),('credit',r'credit|funding|client readiness'),('trading',r'trad|oanda|vibe'),('blockers',r'block'),('status',r'status|summary|system|report')]:
...
def advisor_response(message,ctx=None):
 ...
 'coffee':"Not coffee, but I’m awake. I’ve got the operating picture in front of me: the scheduler is running, the money path is still the $97 readiness journey, and the main blockers are approvals, Resend, and the fake customer insert. But we can talk through it like normal—what are you trying to move first?",
 'approval':"You’ve got a few decisions that actually matter. The big one is the synthetic customer insert, because that unlocks the live dashboard test. Then Stripe test completion, the Resend fix, and selected content or communication drafts. I’d handle them in that order unless you want communication moving first.",
 'empathy':"I get why. The backend kept reporting wins, but the app experience lagged behind. That is exactly the wrong feeling for an operating system. I’d treat today as a UI trust-repair day first: make every visible button do something, make approvals explain themselves, and make me talk with you instead of acting like a terminal wrapper.",
 'partner':"Fair. You don’t need another interface telling you to run commands. You need me to think with you, challenge weak priorities, keep the operating context straight, and take structure off your plate. We can talk first. When you want action, I’ll help turn the conversation into work without pretending the conversation itself is a command.",
 'today_plan':"For today, I’d keep it tight: finish the visible UI trust repair, decide the synthetic customer and Stripe test path, then clear the Resend configuration blocker. I can organize the specialist work behind those three while you focus on the decisions.",
