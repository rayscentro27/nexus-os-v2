...
def envs():
 ...
 present={x:bool(values.get(x)) for x in names}; mode=(values.get("OANDA_ENVIRONMENT") or values.get("OANDA_API_URL") or "").lower()
 return present, ("practice" if any(x in mode for x in ("practice","demo")) else "live_detected_blocked" if "live" in mode else "unverified")
...
def build():
 ...
 demo_ready=mode=="practice" and (present["OANDA_API_KEY"] or present["OANDA_ACCESS_TOKEN"]) and present["OANDA_ACCOUNT_ID"]
 status="oanda_demo_config_present_no_order_test" if demo_ready else "oanda_config_unverified_or_incomplete"
