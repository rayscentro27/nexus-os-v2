...
GoClear / Apex Revenue Hub cards use `task_requests.task_type=goclear_revenue_metric_project` with `owner_tab=goclear`, `project_type=revenue_metric`, `payload.metrics`, `payload.affiliate_stats`, `payload.pipeline_stage`, and `payload.project_enrichment`. These cards represent revenue-pipeline signals or estimates only unless a future safe revenue/referral connector supplies actual values.
