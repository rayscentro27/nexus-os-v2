...
| Watched resource watch | Source Intake | `scripts/research/run_watched_resource_watch.py` | `task_requests`, future `research_sources`, proof | Future daily/weekly after approval | Medium | Required before scheduler activation | `python3 scripts/research/run_watched_resource_watch.py --dry-run --limit 3 --no-external-ai --json` | `watched_resource_update_created` | Ready for schedule, disabled |
| Research/content growth engine | SEO / Marketing | `scripts/research/*` | `task_requests`, `nexus_events`, local reports | Future daily digest after approval | Medium | Required before scheduler activation | `python3 scripts/research/seo_affiliate_content_planner.py --dry-run --limit 5 --no-external-ai --json` | `research_content_growth_candidate_created` | Manual only |
