...
`_tool_execution_state()` now passes current-turn facts to final presentation: search, retrieval, Nexus, and Alpha execution booleans; per-tool counts/statuses; currentness values; and retrieval status.
...
Regression evidence: general preflight used zero tools; Tesla used public search and retrieval; the multi-resource preflight executed both Nexus and web retrieval and its final text did not claim retrieval was pending.
