...
export async function handler(event) {
  try {
    ...
    await db(c, `nexus_tiktok_connections?id=eq.${connection.id}&user_id=eq.${encodeURIComponent(u.id)}&tenant_id=eq.${encodeURIComponent(u.tenant)}&environment=eq.${c.runtimeEnv}`, { method: 'PATCH', body: JSON.stringify(patch) })
