...
export async function handler(event) {
  try {
    ...
    await db(c, `nexus_tiktok_connections?id=eq.${connection.id}&user_id=eq.${encodeURIComponent(u.id)}&tenant_id=eq.${encodeURIComponent(u.tenant)}`, { method: 'PATCH', body: JSON.stringify({ status: 'DISCONNECTED', revoked_at: new Date().toISOString(), updated_at: new Date().toISOString() }) })
